...
- **M29 E #260 管理工作台收口 + 文档 + 三面 parity**：新增 `docs/management-tools.md`，汇总八模型工作台、对标单品、Web Admin / Studio 桌面 / VS Code 交付覆盖度，并按浏览 / 查询 / 编辑 / 导入导出 / 监控明确三面真实边界；加入 Web 多模型与 Studio bridge 两张可复现截图。`web/e2e/management-workbenches.spec.ts` 使用 Playwright + 统一契约 fixture 对 SQL/时序、关系、文档、KV、MQ、向量、全文、对象桶八个工作台和 Studio bridge 做真实 Chromium smoke；VS Code 扩展新增 loopback HTTP consumer smoke，覆盖 schema、SQL NDJSON、KV、向量、全文与 MQ；相关 smoke 已接入独立 CI workflow。核查同时明确：Studio 原生菜单目前仅有 action manifest，对象/备份专用原生文件接线尚不完整；VS Code 保持只读开发者子集，不宣称完整 per-model 编辑面。
