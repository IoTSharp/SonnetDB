...
- **M27 #185 本地 ONNX model profile 证据合同**：新增 [`m27-provider-model-profile.md`](docs/benchmarks/m27-provider-model-profile.md)，冻结 tokenizer、输入 tensor 类型/名称、截断与 padding、输出形状、`mean`/`cls` pooling、L2 归一化和维度隔离语义，并区分合成 tiny ONNX 合同测试与真实目标模型质量/性能证据。新增 `m27-local-onnx-evidence-v1` runner/verifier，绑定模型、tokenizer、profile、corpus 的 SHA-256 与来源/version/license，记录逐查询延迟、向量摘要、Top-K、Recall@K、峰值 RSS/托管内存和 clean commit，并从原始样本重算质量与延迟摘要。当前内置 docs/skills 知识库仍固定为 `VECTOR(384)`，非 384 profile 仅可直调 provider 或使用独立索引；缺失真实资产、hash fallback、低 Recall、脏工作树或摘要篡改均不能形成发布证据，真实报告归档前继续保持 `NOT_READY`。
