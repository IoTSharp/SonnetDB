...
- 新增关系表 `INT AUTO_INCREMENT` 列，兼容 `AUTOINCREMENT` / `IDENTITY` 拼写；省略、`NULL` 或 `DEFAULT` 插入会从 1 开始分配持久化单调递增值，批量、轻事务、触发器和直接 `TableStore` 写入共享同一高水位，显式更大整数会推进序列，`DELETE` 保留而 `TRUNCATE TABLE` 重置序列。普通 INSERT 在受锁提交阶段分配；事务/触发器的提前预留记录 generation，并在并发 `TRUNCATE` 后拒绝陈旧提交。序列采用数据库常见的预留语义，失败或回滚可能留下间隙但不会复用；属性不隐式创建唯一约束，SQL `UPDATE` 不允许显式修改自增列。schema、REST/远程 ADO.NET 元数据、`DESCRIBE TABLE` 与 `information_schema.columns` 同步暴露自增标志；`tables/tables.tblschema` 向后兼容升级为 v8，v1-v7 文件继续可读且旧列视为非自增，写入 v8 后旧版 SonnetDB 不能读取该 schema 文件。当前 `INSERT` 结果尚无 `RETURNING` / last-insert-id，EF Core provider 的整数 `ValueGenerated.OnAdd` 仍使用客户端键生成。
