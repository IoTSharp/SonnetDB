...
- **M29 B #249 可视化 EXPLAIN 第一批**：Web Admin 的 SQL 结果面板会识别 `EXPLAIN` 返回的 `key` / `value` 计划行，新增默认打开的 `Plan` 视图，将既有 access path、索引名、估算行数、time/tag 下推、document planner 候选计划、sort/projection 覆盖情况渲染为可视化计划树与候选路径侧栏；原始 Table / Raw / JSON 视图和 CSV / JSON 导出保留不变。本批只接入可视化 EXPLAIN，#249 的表设计器与索引管理继续后续拆分。
