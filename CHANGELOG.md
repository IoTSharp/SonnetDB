...
- **M37 #327 逻辑视图**：新增持久化 `CREATE VIEW [IF NOT EXISTS] name AS SELECT ...`、`DROP VIEW [IF EXISTS]`、`SHOW VIEWS`、`DESCRIBE VIEW` 与 `information_schema.views`。视图定义保存在独立版本化、CRC 校验并原子替换的 `views/views.sdbview`，启动时重建 AST；SELECT 统一展开为派生表，复用现有关系表、measurement、document、JOIN、UNION 和子查询执行路径。创建时拒绝参数占位符、未知数据源和自引用，执行时防止循环与超过 32 层的嵌套；DROP/ALTER 被引用基础对象及 DROP 被依赖视图会明确失败。服务端只读权限、帧协议路由、MCP 与 Copilot SQL 分类同步识别视图元数据语句。物化视图、SQL 存储过程和触发器按 M37/M38 后续独立条目推进；C# 脚本、动态库和其他外部运行时保持暂停。
