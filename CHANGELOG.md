...
- **M40 Phase 3 #360 statement snapshot 与并发冲突矩阵**：明确 `GraphStore.BeginRead` 创建单一 KV sequence 的 statement snapshot；同一 `GraphReadSession` 的点读、并发提交前后创建的游标和分页长遍历保持一致，cursor lease 不持有 Graph 提交锁。原生 `EXPLAIN ANALYZE GRAPH_TABLE` 新增 `read_consistency=statement_snapshot`、`actual_read_consistency` 与 `actual_snapshot_sequence`；关系映射如实报告 `relation_accessor_current` 和 null sequence，并继续由 M41 #374 负责关系 statement snapshot。新增并发 re-parent、晚建 cursor、不相交 element update、unique property claim、endpoint delete/edge insert 矩阵，验证 writer 不被长读阻塞、冲突恰有一个胜者且图 invariant 无 orphan/index drift。当前不扩展跨 statement snapshot-isolation transaction；后续 #362~#364 功能切片已完成，#365~#367 仍待实施，不构成生产级图数据库发布门禁通过。
