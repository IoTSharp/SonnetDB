...
- 内存表与保留策略并发回归使用固定轮次、专用任务和有界同步，消除忙循环阻塞线程池中写入/取消/续体的风险；仍追加每序列 2,000 点，核对每个读者完成 2,000 次读取，并验证五轮保留与 1,000 次新鲜写入全部完成。
- 段管理器 Add/Drop 并发回归改用专用线程和有界屏障，消除忙循环挤占取消计时器与写入任务的线程池饥饿；50 个读者跨越 40 轮实际增删，逐轮核对新旧索引与 reader 快照、删除后的租约解码和最终释放，失败时等待所有工作线程退出。
- 语义对象桶回填的内部页预算耗尽后，保存已完成对象的精确游标和累计计数并返回可续页状态；末项入队成功后不再因已取消的页 token 返回 HTTP 500。进度提交使用独立五秒窗口并联动调用方取消，存储 I/O 错误继续传播，复用原有对象列表令牌且不改变持久格式。
- 修复查询 reader-map 缓存并发测试的同源线程池饥饿：24 个专用查询任务跨越 40 次实际段替换，每轮同时验证在途旧查询的完整时间戳/值与发布后新查询的缓存刷新，断言全部 1,920 次查询完成并直接传播异常。
- 修复段替换并发测试在线程池饥饿时无法触发停止定时器、导致 Windows CI 单项耗时超过 24 分钟的问题；改为 50 个专用读者与 40 次有界发布同步，实际覆盖 writer 锁内旧快照读取、跨发布 reader 租约及完整数据解码，工作者异常直接使测试失败。
- 墓碑表并发回归改为 50 个写者和 50 个读者的独立线程、固定 32 轮及有界同步，避免线程池忙循环饿死取消计时器；每个参与者都必须完成指定工作，并校验并发快照、覆盖范围和最终全部 1600 条墓碑，工作线程异常直接使测试失败。
...
- 修复 Parity full 的 ClickHouse 默认用户无法远程查询却被 `/ping` 放行的问题：参考容器与客户端统一使用专用测试凭据，并以认证 `SELECT 1` 验证就绪。light/full 已声明的参考服务不可达或没有实际成功执行场景时，汇总门禁失败；能力缺口和未选 profile 的跳过继续单列。
...
### Changed
- 连续七天 scheduled Parity 改为正式发布的非阻断观察项，窗口不足、历史失败或获取失败仍如实记录在报告中；同一候选提交与版本的十二类工作流及当前候选 light/full 原始证据继续作为发布要求。新增实际 verifier 编排回归，覆盖观察不足、不可用和候选失败的区别。
...
- **M27 #340 Studio bridge 安全前置**：Studio 启动 URL 不再携带 bridge endpoint/token，Web 也不再读取或保存 query/storage 凭据；NativeWebHost 通过 request/event 握手把启动配置仅注入当前 WebView 内存，并清理旧版 URL 与 storage 残留。loopback bridge 只接受配置的 Studio origin 与 header token，拒绝缺失/非受信 origin 和 query token，同时保留受信 CORS preflight。该变更不包含 StudioNative AI broker、系统凭据库、外部 OAuth/BYOK 或真实双网验收。

- **Document 过滤式 FullText 公共入口**：新增 extend-only `DocumentCollectionStore.SearchFullTextFiltered`，在 FullText posting 处理阶段应用允许的 document ID 集，返回过滤候选数、posting visit 计数和预算状态；取消与 posting 上限贯穿执行，预算耗尽时不返回部分排序结果。既有 `SearchFullText` 签名和行为保持不变。

- **M27 #340 双网客户端 Copilot runtime**：Web Copilot 新增显式 `ServerRelay` / `BrowserDirect` / `StudioNative` / `Disabled` 模式和统一 transport/事件状态机；每次运行先校验独立的本地/公网 readiness，再以 `runId`、严格 `sequence`、opaque `cursor` 和 `toolCallId` 处理事件顺序、稳定 ID 下的重复调用、工具名绑定、结果后禁止 retry、冲突与取消，并要求唯一的 `final` 或 `error` outcome、且 `final.answer` 非空，随后才能以 `done` 完成。ServerRelay readiness/POST 固定当前 SonnetDB 端点、使用 `credentials: omit` 并以 `redirect: error` 拒绝消息/Token 经 3xx 外送；SSE 严格解析 LF/CRLF/bare CR、跨分片和多行 data。服务端在输出事件或执行工具前拒绝缺 `done`、双 outcome、空 final、终态后事件和同轮 tool/outcome，避免终态后写副作用；并在单次运行内按稳定 `toolCallId` 缓存工具名、参数与本地结果，精确回放只重发缓存结果，同 ID 名称/参数冲突则在整轮预检阶段拒绝，保持事件顺序且不重复执行。Dock 在停止、关闭、切换/删除会话、登出和卸载时取消请求，清除未完成回答并从服务端重同步会话；SQL 页签只在完整 `done` 后提交。BrowserDirect 已从显式 HTTPS 公网 URL 和批准 origin 注册到真实聊天入口，公网 token 仅驻留内存、强制未来过期且 TTL 不超过两小时，并在与数据库 token 同值、配置缺失、过期或登出时 fail closed，不回退 ServerRelay；Web 现从当前数据库连接派生固定 `/mcp/{db}`，使用独立数据库 Bearer 完成 typed `initialize` / `tools/list` / `tools/call`，只允许显式出域 allowlist 中完整声明 read-only、non-destructive、idempotent、closed-world 的工具。客户端校验 input/output schema、typed v1 成功/错误合同、64 KiB 结果和 8 次 loop 上限；本地错误不发送 continuation，公网下一段必须逐字回显结果。同 `toolCallId` 同规范参数只回放缓存结果且不二次执行，冲突在执行前拒绝，重复回放仍计入 loop 预算。ServerRelay 已切换为服务端稳定 envelope 与同进程重放，不再使用当前流 FIFO 合成 ID；可信 OAuth/PKCE、已部署公网 continuation/CSP/CORS、StudioNative、页面刷新/进程重启/多实例续流和真实双网验收仍未实现。

- **M35 #298 metadata-filtered ANN 首切片**：语义图片集合新增 source bucket path index 与 metadata/tag wildcard index，旧集合一次 schema apply 原子补齐三个派生索引；过滤查询选择最小索引候选并按 256 行分页、逐页观察取消，再由 managed HNSW 遍历图并只接纳允许 ID。候选不足时，小集合做精确补偿；超过 4096 项后不再无界保留 ID/文档集合，改为沿同一索引分页计算 exact top-K；仅含 Key Prefix/Content-Type 等不可索引条件时分页扫描精确回退。HNSW 派生图缺失 allowed key 时显式要求精确回退，不把索引漂移静默表现为少返回；`backend`/`searchMode` 如实区分 `managed`、`prefiltered-ann`、`prefiltered-ann-exact-compensation` 与 `exact-filtered-fallback`。显式 `usearch` 且关闭 managed fallback 时，过滤查询返回 503，不再静默切换后端。USearch filtered API、可配置预算与固定硬件质量/容量证据仍未完成，#298 保持进行中。
...
- **M27 #185 本地 ONNX model profile 证据合同**：新增 [`m27-provider-model-profile.md`](docs/benchmarks/m27-provider-model-profile.md)，冻结 tokenizer、输入 tensor 类型/名称、截断与 padding、输出形状、`mean`/`cls` pooling、L2 归一化和维度隔离语义，并区分合成 tiny ONNX 合同测试与真实目标模型质量/性能证据。`m27-local-onnx-evidence-v2` runner/verifier 绑定模型、tokenizer、profile、corpus 的 SHA-256 与来源/version/license，记录逐查询延迟、向量摘要、Top-K、Recall@K、峰值 RSS/托管内存、环境名、实际 batch、线程请求/应用状态和 clean commit，并从原始样本重算质量与延迟摘要；另冻结 blank、CJK、超长、attention-mask/padding、batch 与 malformed-model 原始边界。v1 报告、线程状态与 session 不一致、缺边界或伪造 batch PASS 均为 `INVALID`；未显式声明并实际加载目标模型或任一边界未运行均不能 PASS。当前内置 docs/skills 知识库仍固定为 `VECTOR(384)`，非 384 profile 仅可直调 provider 或使用独立索引；缺失真实资产、hash fallback、低 Recall、脏工作树或摘要篡改均不能形成发布证据，真实报告归档前继续保持 `NOT_READY`。

- **M20 Parity nightly 证据验证器**：新增只读 PowerShell verifier，通过 GitHub Actions artifact 或同构离线 fixture 检查最近至少七次 scheduled run 的 `light` / `full` 双 profile、完整 schema v2 字段与计数不变量、run/commit 绑定，并将 summary suites 与 `raw/<runId>/report.json` 的实际场景数一一对账。`RequiredRunCount` 下限固定为 7，只能扩大窗口；默认 `NOT_READY` 会失败退出。离线合同覆盖当前“三次成功 + 四次失败”、不足七次、缺字段/`gap_reason`、计数或 raw 对账不一致和七次连续成功。2026-08-29 的真实 artifact 审计结果仍为 `NOT_READY`（4/7，57.14%），还需三次连续成功；研发实现已完成，M20 状态为 ✅（待验证）。
...
- **对象调度分页 CI fixture**：历史终态背景记录改用有界批量写入，避免 Windows runner 为准备数据执行 4,096 次独立 WAL 同步；保留全部 4,096 条终态正文、七天 TTL、真实到期/未来任务写入及仅访问 8～9 个候选的分页断言，不扩大原有 90 秒期限。
...
- **M27 #185 embedding preview 错误合同**：`/v1/db/{db}/vector/embed-preview` 将本地 provider 的 `InvalidDataException` 与 tokenizer/profile 的 `ArgumentException` 按稳定的 `503 embedding_failed` 响应返回，避免合同错误泄漏为未结构化的 500；新增端到端回归测试。
...
- **关系表冷开启动预热**：Server 自动加载已有数据库后，会在业务端口就绪前逐库打开全部关系表，提前完成索引恢复、行数统计和状态加载；不同表的独立 KV 恢复默认使用四并发，并通过 `RelationalTableWarmupConcurrency` 限制在 1～16，管理器在并行阶段继续阻止查询和 DDL 观察半完成状态；新增 `relational_table_warmup` readiness 门禁与完成数量，预热失败时保留诊断入口但不再让首个业务查询在 `TableManager` 全局锁后承担冷开阻塞。
...
- **M40 修复与发布步骤 3，Phase 1 合同补齐**：`GraphVertexPredicate` 与新增 `GraphReadSession.Expand` 重载支持按目标顶点 label 及 typed property 等值过滤，`GraphExpandRequest` 同步扩展 `targetLabelId` / `targetPropertyId` / `targetPropertyValue`；嵌入式、HTTP 与 typed SDK 共用同一邻接 cursor 语义，带过滤请求在 Frame v1 配置下显式回退到 source-generated HTTP 流，旧帧布局保持不变。Graph import 现在同时受 10,000 元素与 8 MiB 规范化 JSON 硬预算约束，`SndbGraphImporter` 按实际 UTF-8 字节分批，以默认 1 MiB 有界读取器拒绝超长 CSV 行，并在完整输入校验/spool 后才发布批次；Server 对已知长度和分块请求均返回稳定 `graph_import_budget_exceeded`。新增 1,000 度过滤分页/分配、字节分批、后续超长 CSV 无部分发布、Frame fallback、HTTP 413 与 SDK 超限回归。#352 固定硬件、外部对拍和发布证据仍为 `NOT_RUN`。

- **M40 修复与发布步骤 2，#367 strict evidence evaluator**：Production manifest 升级为 source-generated `m40-graph-production-input-v2`，dataset、environment、soak、journey 与 check/closed-gap artifact 使用独立 schema；判定器从三轮逐样本 latency/allocation/GC/I/O/访问计数、oracle assertion、checkpoint、真 kill/reopen、cold-open 和资源样本重算所有摘要，不再信任 manifest 自报 `PASS`。artifact 绑定真实 commit/HEAD 和 clean worktree，结构化 executable/argument/working-directory 命令不经过 shell，必须携带 `{artifact}` 并限制为仓库内 `dotnet run` harness 或当前 verifier，再实际回放退出码；回放前后重复校验 SHA-256。新增 allocation P95 不超过 query memory、每 1,000 样本 Gen0/Gen1/Gen2 不超过 100/10/1 次、GC pause P99 不超过 50 ms 的公开门禁，以及伪 `{ "status": "PASS" }`、缺样本、摘要漂移、复现失败、无效 commit 和脏工作树回归。步骤 3~4 已由后续条目关闭；步骤 5~7、固定硬件、外部对拍和 168 小时 Production evidence 仍未完成。

- **M40 修复与发布步骤 1，Graph 正确性阻塞项**：`Expand(Both)` 现在会保留 Out 切换到 In 时尚未消费的底层 KV 页，避免 page size > 1 时丢失入边；Out/In/Both、self-loop、parallel edge、BFS、SQL 内部 256 条分页和远程 typed SDK 均增加回归。无权 shortest path 新增独立 `MaxPaths` 请求预算，并用额外一条探测区分完整不可达与预算截断；耗尽时 Core 抛出 `GraphTraversalLimitExceededException`，HTTP/SDK 稳定返回并解析 `graph_budget_exceeded`，不再把可达目标伪装为 `null`。步骤 3~4 已由后续条目关闭，当前下一门禁为步骤 5；#352 和 Production gate 未完成。
...
- **M41 #372 关系输入下推**：关系 JOIN 在读取基础表时按唯一列归属拆分顶层 `AND`，把可证明的单表谓词交给既有主键/二级索引候选规划，并在 JOIN 行复制前完成完整谓词复检和所需列裁剪；原始顶层 WHERE 始终保留。外连接 WHERE、跨输入表达式、歧义绑定、相关子查询、含标量函数的谓词、逻辑视图/派生表等无法证明等价的形状保守回退。无排序、聚合、DISTINCT、WHERE 或子查询的纯 LEFT JOIN 可把 `OFFSET + FETCH/LIMIT` 安全窗口下推到左输入；零窗口不再生成首行。新增执行计数与 9 项差分回归，覆盖双侧索引、跨输入残余、LEFT JOIN NULL、聚合、相关子查询、逻辑视图、安全 LIMIT、事务 overlay 和有状态 UDF。30,000 任务/128 设备的本地 BenchmarkDotNet 对拍中，均值由 128.44 ms 降至 61.61 ms，分配由 30.18 MB 降至 17.94 MB；固定硬件、木垒生产复测和发布 gate 仍为 `NOT_RUN`。
...
- **M41 #369 EXISTS/IN 与 semijoin 快速路径**：普通单表 `EXISTS` 复用主键、二级索引和有界索引并集候选，并在首个真值行停止；单列正向字面量 `IN` 及可证明非相关的单表 `IN (SELECT ...)` 会先去除 NULL 和重复物理键，再在一个稳定 KV 快照内执行主键或单列普通索引 MultiGet。完整残余谓词、SQL NULL 三值逻辑、参数、相关子查询和事务 overlay/read-your-writes 保持不变；无法证明等价的形状继续走关系执行器。运行时与 `EXPLAIN` 共用 semijoin 重写和 access path 判定，统一报告 `primary_key_in` / `secondary_index_in`、索引、残余和 fallback。
- **M41 #370 有界索引并集**：顶层 AND 中的单个 OR 可把最多 32 个主键、正向 IN、二级索引范围/等值或 indexed `IS NULL` 分支按主键字节去重；规划遍历与候选集合分别受 32 分支和 65,536 行固定上限约束，超过阈值会丢弃部分集合并完整回退，不会静默截断。全部分支共享一个稳定 KV 快照，避免为每个 OR 分支重复复制 mutable overlay 并保持同一可见性时刻。恒 FALSE/UNKNOWN nullable 参数分支会被消去，恒 TRUE、多个 OR 并集、不可索引分支和事务写集均提供稳定 fallback reason；既有 PK/IN/单索引访问路径优先，避免 `A AND (B OR C)` 被更宽并集抢占。
- **M41 #371 双向索引 cursor 与倒序 Top-N**：KV 稳定快照可在 mutable、frozen 和 disk 三层直接反向 seek/merge，降序 continuation 严格读取更小 key；关系 Int64/DATETIME 范围按逻辑符号段倒序遍历。满足非空排序键、完整索引顺序、无残余/DISTINCT/事务 overlay 且分页上限有限时，`ORDER BY ... DESC LIMIT/OFFSET` 把窗口下推到反向 cursor 并早停；多列倒序、NULL 顺序或其他无法证明的形状继续走现有全量排序/有界 Top-N。
...
- **M40 Phase 3 #366 Graph 运维产品面**：新增 Server Graph operations overview、有界 statement-snapshot visualization、基于 `PipeWriter` 的流式 importer-compatible JSON export，以及 repair/rebuild、checkpoint、compact 的 10 分钟两阶段审批和 durable NDJSON 审计；`Read`、`Write`、`Admin` 权限分别约束读取、受限编辑/导入和维护决策。`SndbGraphClient` 在嵌入式/远程模式公开一致的 overview、visualization、export、stage/approve/reject/audit API，嵌入式审计持久化后支持跨 CLI 进程批准；`sndb graph` 新增 list/overview/visualize/export/import 和 maintenance 命令。Web/Studio Explorer 接入 Graph 对象和 URL workspace，Graph Workbench 提供 ECharts Canvas、schema/index/degree/slow traversal、带版本条件的 vertex/edge 编辑、导入导出截断警告、staged maintenance 和审计，并覆盖桌面/移动端真实 Canvas 像素回归。所有 JSON 继续使用 source-generated context，Graph V1 record/WAL 未变；#367 固定硬件、外部对拍、7 天 mixed workload、恢复/容量和 Production/Couplet 联合发布门禁保持 `NOT_RUN`。

- **M40 Phase 3 #365 知识图谱与 GraphRAG 上层合同**：新增 schema v1 的 Entity/Alias/Claim/Source/Chunk/Community/Summary、provenance、0~1 confidence、半开 valid time、Document/Object 权威内容引用和 Vector record/profile 引用，并以严格校验拒绝混合 claim object、错误关系形状、缺失 chunk、非 UTC 时间、正文级 literal 和无界批次。`KnowledgeGraphMapper` 使用固定 `m40-kg-v1` 稳定 ID/label/property 投影，把最多 256 个节点与关系编译为既有 `GraphImportRequest`；`ImportKnowledgeGraphAsync` 在嵌入式与远程模式复用同一幂等 Graph transaction，不修改 Graph V1 record/WAL 或新增 endpoint。公开 source-generated `KnowledgeGraphJsonContext` 保持 AOT JSON 边界；抽取、消歧、事实判断、embedding 和 LLM/community summary job 仍留在 Server/SDK/上层，Document/Object/Vector 继续保存权威内容和派生向量。Core、嵌入式和远程回归已覆盖；#367 固定硬件/长稳/发布门禁保持未完成。

- **M40 Phase 3 #364 受限 GQL 风格直接查询入口**：新增显式 opt-in 的 `GqlParser.Parse` 与 `SqlExecutor.ExecuteGql` 嵌入式 API，支持 `USE GRAPH ... MATCH ... RETURN`、出/入/无向固定模式、1~64 hop 的 `WALK`/`TRAIL`/`SIMPLE`/`ACYCLIC`、`ANY SHORTEST`、变量属性谓词、参数、显式投影、去重、排序/分页和 `EXPLAIN [ANALYZE]`。GQL 与 SQL/PGQ 共用同一个 MATCH parser，并直接生成既有 `GraphTableSource`/`SelectStatement`，不新增 Graph 执行器、存储、权限或 wire 旁路；原生图和关系映射图的 typed AST、计划和结果对拍已覆盖。公开能力矩阵明确拒绝 GQL DDL/DML、多 pattern、`RETURN *`、Cypher `:Label`/`MERGE`/`WITH` 等未承诺语法；完整 GQL/Cypher、远程专用 endpoint 和 #367 Production gate 不在本项范围。

- **M40 Phase 3 #363 可恢复离线图算法**：新增固定 statement snapshot 的 degree、精确 weakly connected components、PageRank 和确定性 label-propagation community 共用 runner；CRC/原子 sidecar 按采集页、完整迭代和发布批次 checkpoint，取消可续作，采集期间 source sequence 漂移明确拒绝。定长状态超过预算后使用 little-endian file-backed vector，community vote 使用有界排序 run 与最多 32 路多轮 merge。结果携带 `operationId@sourceSequence` 版本，可幂等写入标准关系结果表或显式 vertex properties；完成后清理算法 spill，不保留第二份常驻图状态。Core Graph 回归覆盖正确性、重开、取消、预算和两类输出；Graphalytics/LDBC、1m/10m、固定硬件、7 天 mixed workload 与 #367 发布门禁继续保持 `NOT_RUN`。

- **M40 Phase 3 #362 加权路径收益证据 runner**：新增固定 seed 的 topology journey fixture、BenchmarkDotNet Dijkstra/A*/双向 Dijkstra 对拍与 source-generated JSON/Markdown 报告，记录 statement snapshot、path digest、native adjacency access path、P50/P95/P99、分配、working set 和 expanded vertex/edge。quick topology 本地矩阵证明 A* 与双向搜索的局部 expanded-edge/P95 准入；Couplet/1m-10m 真实语料、固定目标硬件、7 天 mixed workload 和 #367 发布门禁继续如实保持 `NOT_RUN`。
...
- **M40 Phase 3 #362 加权最短路径与批量算法执行**：Graph statement snapshot 上新增带边属性/selector 权重的 Dijkstra、显式启用的 A*、双向 Dijkstra 和共享快照批量执行器；路径结果携带总权重、实际算法和扩展计数。负权、缺失/错误类型、NaN/Infinity、累加溢出、frontier/访问/扩展预算和取消均稳定 fail fast。新增 `/weighted-shortest-path` source-generated HTTP/typed SDK 合同，嵌入式与远程路径结果保持一致；本项不宣称 #367 固定硬件、7 天 mixed workload 或 Production gate 已通过。
- **M40 Phase 3 #361 supernode 治理与可恢复维护**：Graph V1 邻接继续按紧凑 key 分条保存；KV state v5 在固定 restart 间隔下对有序 key 做前缀压缩并保留 v1-v4 读取兼容。新增 `GraphStore.RunMaintenance`、`GraphMaintenanceOptions/Result` 和 CRC/原子替换的 `maintenance.sdbgraph` continuation，按页 repair 邻接/label/property/unique 派生索引，页间释放提交门，支持取消、重开续作、最终 checkpoint 与显式 compaction；新增 `GraphStore.Checkpoint/Compact`。统计刷新按 outgoing anchor 流式生成 degree histogram，并受扫描/分组预算约束。100,000-degree supernode、坏 sidecar、v4 state 兼容和恢复回归已覆盖；固定硬件与 7 天生产 gate 仍归 #367，未宣称 Production 通过。
- **M40 Phase 3 #360 statement snapshot 与并发冲突矩阵**：明确 `GraphStore.BeginRead` 创建单一 KV sequence 的 statement snapshot；同一 `GraphReadSession` 的点读、并发提交前后创建的游标和分页长遍历保持一致，cursor lease 不持有 Graph 提交锁。原生 `EXPLAIN ANALYZE GRAPH_TABLE` 新增 `read_consistency=statement_snapshot`、`actual_read_consistency` 与 `actual_snapshot_sequence`；关系映射如实报告 `relation_accessor_current` 和 null sequence，并继续由 M41 #374 负责关系 statement snapshot。新增并发 re-parent、晚建 cursor、不相交 element update、unique property claim、endpoint delete/edge insert 矩阵，验证 writer 不被长读阻塞、冲突恰有一个胜者且图 invariant 无 orphan/index drift。当前不扩展跨 statement snapshot-isolation transaction；后续 #362~#366 功能切片已完成，#367 发布门禁仍待实施，不构成生产级图数据库发布门禁通过。
- **M40 Phase 2 #353~#359 功能闭环**：新增 typed `GraphLogicalPlan` 与共享 pull 执行入口；原生 Graph seek、全量 scan、Expand、BFS/DFS/path 均可从同一计划执行。SQL 新增 `CREATE/DROP/SHOW/DESCRIBE GRAPH`、参数绑定的 `graph_nodes`/`graph_edges`、WHERE/投影/ORDER BY/分页/EXPLAIN，以及受限 `INSERT INTO GRAPH ... VERTEX|EDGE`，mutation 复用同一 `GraphStore`/`GraphTransaction`。新增不复制关系行的 `CREATE/DROP/SHOW/DESCRIBE PROPERTY GRAPH`、CRC catalog 与 `RelationalGraphAccessor`，校验唯一 key 和 endpoint reference，并在 EXPLAIN 中显示关系主键/索引 seek 或有界 scan fallback。`GRAPH_TABLE(... MATCH ... COLUMNS (...))` 支持原生 adjacency 与关系映射固定一跳、出/入/无向方向、label、变量属性谓词、参数、投影和外层 SQL shape；同 label 多映射按 branch union，无向自环去重，整条关系映射查询共享 10,000 anchor、10,000 fallback scan 行、50 ms fallback 和 100,000 匹配行预算。#357 增加 `{min,max}`（1~64 hop）、`WALK`/`TRAIL`/`SIMPLE`/`ACYCLIC`、`ANY SHORTEST` 和 `length/vertex_ids/edge_ids/start_id/end_id` 路径投影；原生图继续执行 `GraphPathPlan`，关系映射图继续经主键/索引或全局有界 fallback 读取，最小深度大于 1 时按首条合法最短路径选择终点。#358 增加 `graph_cost_v1`：按端点 key、关系表实时行数、edge index/fallback 与路径深度估算左右 anchor/expand 成本，可从右端反向执行并恢复原始路径投影方向；`EXPLAIN ANALYZE GRAPH_TABLE` 实际执行只读查询并报告 rows、anchors、expansions、generated paths、peak frontier、fallback rows/ms 和总耗时。#359 复用标准派生表和现有关系执行器，在同一 SQL 中组合 Graph + Table/Document/FullText/Vector Hybrid 候选，不新增应用层 merge 或第二套 JOIN；组合 `EXPLAIN` 展示逐源 access path、候选合同与 fallback。property-graph catalog 获得 manifest 完整 mapping 摘要、独立文件分类和 restore 后复核；Graph SQL read/write 权限分流，Studio Quick SQL 继续走 Server 共享结果面板，Parity 增加 Graph Beta capability 和本地 correctness scenario。双向 BFS 只有真实基准证明收益才准入，当前明确返回 `benchmark_evidence_missing` 而不引入第二套算法。Graph DDL/DML 在轻事务中明确拒绝，view/materialized view/procedure 记录真实 graph 依赖并阻止悬空 DROP；结果已通过直接 Frame、远程 ADO.NET REST、跨模型 Core、备份恢复、鉴权和 Web build 回归。固定硬件、PostgreSQL/Neo4j 外部对拍、Couplet C3 联合 gate 与完整报告保持 `NOT_RUN`，不宣称 Beta 已发布。
- **M40 Phase 1 功能闭环**：#347~#351 的 Native Graph Core、REST/NDJSON/Frame、typed SDK、幂等 CSV/JSON importer 和本机 correctness smoke 已完成；#352 固定硬件、Neo4j、完整恢复/容量与发布决定继续标记为后置证据，不将本机结果冒充发布 gate。
...
- **M40 Phase 0 #344 Graph 元数据与生命周期**：新增 `GraphCatalog`、`GraphManager`、`GraphStore`、独立目录以及 `Tsdb.Graphs`，覆盖 create/open/reopen/drop、跨基础对象命名与依赖阻断、固定长度 marker 的有界损坏拒绝、单 owner 释放和同一 graph root 的进程内/跨进程 lifecycle lease；catalog 原子替换结果不确定时保留完整 store、停用当前 manager 并要求重开裁决，Phase 0 仍不公开 vertex/edge 查询能力。
- **M40 Phase 0 #345 单 graph 原子事务合同**：新增内部乐观事务、element version 条件提交、幂等 request、commit-unknown 结果、顶点删除 `RESTRICT` 和展开后的操作/属性/WAL 字节预算；冲突、取消、无限属性输入、超限与 WAL 故障均在条件原子 batch 边界内保持零半边、零孤立邻接和确定性重试语义。
- **M40 Phase 0 #346 Graph 恢复与备份骨架**：backup manifest 升级为 v2 并继续读取无 Graph 内容的 v1；新增 Graph checkpoint、备份/提交共享 gate、manifest verify、restore 发布前 reopen + invariant check，以及检测 orphan/mismatch 的校验器和 CrashTests 注入骨架。v1 manifest 若声称携带 Graph 内容会被拒绝，恢复不会把未校验目录发布为目标数据库。
...
- **M34 Modbus TCP 内建映射表 Phase A（#288~#290）**：新增 `CREATE MODBUS SOURCE/ENDPOINT`、列级 `FROM MODBUS` / `EXPOSE AS MODBUS`、表级 `USING MODBUS` 及 `SHOW/DESCRIBE MODBUS`；定义持久化到独立的版本化、CRC 校验、原子替换 catalog，并接入备份恢复和表依赖保护。实现零基/一基/Modicon 地址归一化、四类地址空间的跨度与冲突校验，以及 BIT、整数、浮点、BCD、定长 ASCII STRING 的严格编解码、字节序/字序和缩放反算。DDL 仅限当前数据库的 Admin，元数据查询只需 Read；Phase A 不包含 TCP 轮询、远端写寄存器、从站监听、外部写审批、诊断或管理面，普通 `SELECT` 与元数据查询不会访问 PLC。
- **M40 原生属性图数据库规划**：新增原生属性图专项路线，明确 Vertex/Edge/Label/Property 与双向 adjacency 作为一级 KV 持久结构，边记录、双向邻接和索引使用单 keyspace 原子 batch；按公共地基、Native Graph Preview、SQL/PGQ Graph Beta 和生产级单机图数据库四阶段推进。原生图与关系表 SQL/PGQ 映射共享 Graph Logical Plan 和流式算子，但分别使用 native adjacency 与 relation accessor；复用现有 WAL/checkpoint、sortable scalar codec、SQL 表达式、Server/SDK、Backup、CrashTests、Parity 和 Workbench，不新增第二套存储、查询、权限或备份体系。正式九模型定位须等待容量、崩溃恢复、SQL/PGQ、Native AOT 和发布证据通过。
- 新增关系表 `INT AUTO_INCREMENT` 列，兼容 `AUTOINCREMENT` / `IDENTITY` 拼写；省略、`NULL` 或 `DEFAULT` 插入会从 1 开始分配持久化单调递增值，批量、轻事务、触发器和直接 `TableStore` 写入共享同一高水位，显式更大整数会推进序列，`DELETE` 保留而 `TRUNCATE TABLE` 重置序列。普通 INSERT 在受锁提交阶段分配；事务/触发器的提前预留记录 generation，并在并发 `TRUNCATE` 后拒绝陈旧提交。序列采用数据库常见的预留语义，失败或回滚可能留下间隙但不会复用；属性不隐式创建唯一约束，SQL `UPDATE` 不允许显式修改自增列。schema、REST/远程 ADO.NET 元数据、`DESCRIBE TABLE` 与 `information_schema.columns` 同步暴露自增标志；`tables/tables.tblschema` 向后兼容升级为 v8，v1-v7 文件继续可读且旧列视为非自增，写入 v8 后旧版 SonnetDB 不能读取该 schema 文件。关系表 `INSERT` 同步支持列名列表或 `*` 的 `RETURNING`，ADO.NET 可用 `ExecuteScalar("INSERT ... RETURNING id")` 获取语句级 last-insert-id，嵌入式、REST/远程结果均保留返回行和实际影响行数；EF Core provider 现在把常规 `int` / `long` `ValueGenerated.OnAdd` 映射为数据库 `AUTO_INCREMENT`，仅用临时跟踪键并在保存后用 `RETURNING` 的数据库值替换，`ValueGeneratedNever()` 不变。
...
- **M39 #333 SQL 触发器 V2 gap baseline（第二批）**：证据 runner 与 source-generated v3 报告扩展为 INSERT/UPDATE/DELETE × 1/100/10,000 行 × 无触发器/V1 行触发器/客户端事务参考的完整成功和失败回滚矩阵；UPDATE/DELETE 在计时前预置并 checkpoint 同规模数据，成功样本核对源表与审计表精确内容，失败样本统一以 sentinel 约束注入并逐行验证源值和审计基线恢复。报告新增 operation、通用 affected rows、有符号 rowstore delta 和双表恢复状态，xUnit quick-contract 使用 1/3 行固定 18+18 个成功/回滚样本及 JSON/Markdown schema；固定目标硬件复测仍未归档，不提前宣称 statement trigger、transition table 或跨 keyspace 掉电原子性。
...
- **M37 #327 逻辑视图**：新增持久化 `CREATE VIEW [IF NOT EXISTS] name AS SELECT ...`、`DROP VIEW [IF EXISTS]`、`SHOW VIEWS`、`DESCRIBE VIEW` 与 `information_schema.views`。视图定义保存在独立版本化、CRC 校验并原子替换的 `views/views.sdbview`，启动时重建 AST；SELECT 统一展开为派生表，复用现有关系表、measurement、document、JOIN、UNION 和子查询执行路径。创建时拒绝参数占位符、未知数据源和自引用，执行时防止循环与超过 32 层的嵌套；DROP/ALTER 被引用基础对象及 DROP 被依赖视图会明确失败。服务端只读权限、帧协议路由、MCP 与 Copilot SQL 分类同步识别视图元数据语句。SQL 存储过程和触发器按 M38 后续独立条目推进；C# 脚本、动态库和其他外部运行时保持暂停。

- 新增 SigLIP2 ONNX 多模态 embedding provider 与语义图片 REST API，支持图片摄取、原图/缩略图读取、文搜图、图搜图、similar-by-id、metadata/tag/source 过滤、按需 explain 和删除；原图复用 Object Bucket，profile 隔离的元数据与向量复用 Document/HNSW 持久化索引。ANN 默认使用 `auto`：官方 USearch 原生资产可用时启用 USearch，其他 RID 或加载失败时回退到纯托管 HNSW；可索引的 source/metadata/tag 过滤使用 managed HNSW 预过滤 ANN 和有界精确补偿，其余条件或大候选不足时回退精确扫描。Bucket 新增默认关闭的异步语义摄取与 WebP 缩略图选项，并提供 KV 持久化任务、幂等版本、退避重试、取消/替代、有界队列背压补偿、重启恢复、补录和处理状态接口；新增 provider/实际后端状态发现和 HTTP 端到端测试，不把大模型下载纳入 Server 启动路径。
...
- 关系表 `UPDATE` / `DELETE` 新增以普通关系表或 measurement 为数据源的非相关单列 `IN (SELECT ...)` 与 `NOT IN (SELECT ...)` 执行：子查询一次物化，支持参数、排序、分页、空集和 `NULL` 三值逻辑；相关、多列或未建立静态绑定合同的数据源在重型扫描前明确拒绝。正向单列主键 `IN` 使用点读，访问路径回归测试固定木垒 `TranCleaner` 形状 SQL 仅保留内层一次扫描、外层不再二次全表解码；远程 ADO 回归同步覆盖字符串主键、时间参数和有界批量删除。
...
- **M32 Document MongoDB-like 易用性闭环**：补齐 `$mul/$pop`、原子 `findOneAndUpdate` before/after、`$elemMatch/$regex/$type/$size/$all`、递归 `$not`、数组 path fan-out 和基础 ordinal collation；path index 新增 multikey/wildcard、parallel-array 拒绝、typed key、EXPLAIN 与崩溃后派生索引修复。Aggregation 扩展 computed project/group expression、push/addToSet 与完整 unwind；Core、HTTP、.NET SDK 和 Web/Studio 统一 mixed ordered/unordered Bulk、逐项结果、严格 operation shape、稳定 `batch_too_large`、单 collection 原子边界及 24 小时 `requestId` 重放。旧 filter payload 的组合优先级及 group-key 非空属性/解构合同保持兼容。新增 `sndb document import`，支持流式 JSON array、有界严格 UTF-8 NDJSON、常用 Extended JSON、mongodump BSON 子集、dry-run、以累计计数和最多 1000 条错误样本保留失败状态的 checkpoint/resume、索引建议和机器报告；新增可嵌入/远程运行的 Document Quickstart、OpenAPI 更新及 `document-mongodb-gap.json` 权威差异清单。MongoDB wire/Driver、replica set/sharding、跨 collection 事务、locale collation、完整 aggregation/BSON 与 positional array update 仍不属于兼容承诺。
...
- **M25 #173 MongoDB 参考 parity 套件**：`tests/SonnetDB.Parity` 新增 SonnetDB `DocumentAdapter`、官方 `MongoDB.Driver` 驱动的 `MongoAdapter` 与 MongoDB 8.0 reference container，覆盖 CRUD、filter/projection/sort、局部 update operators、unique/TTL index、aggregation、并发写、进程重开恢复和索引一致性；双端结果进入既有 JSON/Markdown diff，MongoDB 不可达时以带 `gap_reason=mongodb_unreachable` 的结构化 SKIP 报告。该套件只验证 MongoDB-like 语义，不声明 wire protocol、BSON command 或官方 Driver 直连兼容。
- **Web Admin 多模型文件写入收口**：KV 工作台新增保留 Base64 原值和逐 key expiry 的 JSONL round-trip 导出/导入；SonnetMQ 工作台新增 JSON/JSONL/NDJSON 消息文件导入，支持逐条 topic、headers、Base64 或 JSON payload 并按文件顺序审批发布；Vector 工作台新增数据编辑/导入页，复用 Measurement 点身份、批处理与停止/续传语义，补齐 VECTOR 有限数值、schema 维度校验和安全 SQL 字面量；FullText 工作台新增面向当前索引集合的独立 JSON/JSONL/NDJSON 导入入口，继续通过 Document API 维护主数据和派生索引。所有写入均保留 bearer grant、`WriteApprovalPanel`、结果与历史边界。
- **M24/M32 Document 管理面闭环（#170~#172 / #279 / #281 界面交付）**：Document Collection Workbench 新增可视化 filter 构建器、服务端同执行器 update preview、单条/批量局部更新共享审批、100 条分批导入进度与停止后续批次、复合/Unique/Sparse/Partial/TTL 索引设计器与一致性校验，以及 collection 级 Change Feed Viewer。Change Feed 在 Document KV/WAL 中持久化 insert/update/delete 序列与前后镜像，提供 7 天保留、操作/文档过滤、24 小时绑定数据库/集合/过滤指纹的 resume token、过期检测和只读权限边界；Web 查看器支持从当前/保留窗口开始、暂停续传、频率选择和前后差异。VS Code Collection 节点同步新增只读 Document find 面板与游标分页。新增专用 HTTP 契约、AOT source-generated JSON 注册、重启恢复核心测试和 Document API 端到端测试。
...
- **M31 #269~#271 时序聚合类型语义增强**：`IAggregateFunction` 新增兼容式 `AcceptedFieldTypes` 能力声明，内置聚合按 numeric / selector / categorical / vector / geo 语义做函数级类型校验；`first/last` 改用保留时间戳与原始 `FieldValue` 的累加器，支持 Float64、Int64、Boolean、String、Vector、GeoPoint；`min/max/mode/distinct_count` 扩展到 String 与 Boolean，字符串比较固定为 Ordinal。补齐分桶、非分桶、flush 后读取、原始类型与错误语义测试，并加入 IoTSharp `ST_DeviceVal` 字符串遥测 60 秒分桶兼容验收、SQL reference 和 Copilot 聚合技能说明。
- **M29 E #260 管理工作台收口 + 文档 + 三面 parity**：新增 `docs/management-tools.md`，汇总八模型工作台、对标单品、Web Admin / Studio 桌面 / VS Code 交付覆盖度，并按浏览 / 查询 / 编辑 / 导入导出 / 监控明确三面真实边界；加入 Web 多模型与 Studio bridge 两张可复现截图。`web/e2e/management-workbenches.spec.ts` 使用 Playwright + 统一契约 fixture 对 SQL/时序、关系、文档、KV、MQ、向量、全文、对象桶八个工作台和 Studio bridge 做真实 Chromium smoke；VS Code 扩展新增 loopback HTTP consumer smoke，覆盖 schema、SQL NDJSON、KV、向量、全文与 MQ；相关 smoke 已接入独立 CI workflow。VS Code 保持只读开发者子集，不宣称完整 per-model 编辑面。
- **M29 E #259 VS Code 多模型消费**：`extensions/sonnetdb-vscode` 从占位骨架推进到可消费 M29 契约的开发者只读面；连接 profile 持久化到 VS Code `globalState`，token 继续存放在 `SecretStorage`。Explorer 在数据库节点下并行加载 schema 与 #245 管理契约，展示 KV Keyspaces、Vector Indexes、FullText Indexes、MQ Topics，并支持 KV/MQ 只读预览、向量 search-preview、全文 search-preview/analyze。Query Result Webview 替换占位页，提供 Table / Raw / Chart 三视图并复用于多模型预览；Copilot 面板接入 `/v1/copilot/chat/stream` 与 `/v1/copilot/models`，默认 read-only，切换 read-write 前走 VS Code modal 确认。VS Code 定位保持“开发者只读浏览 + SQL 执行子集”，完整 per-model 编辑仍归 Web Admin / Studio。
- **M29 E #258 Studio 桌面原生桥**：`SonnetDB.Studio` 新增只监听 loopback 的本地 `studio-bridge` HTTP 控制面，启动期随机 token 通过 WebView URL 注入，前端请求统一带 `X-SonnetDB-Studio-Bridge-Token`；桌面壳提供原生打开 / 保存文件对话框、磁盘持久化连接库、本地 `data root` 托管 SonnetDB Server 启动 / 停止 / `/healthz` 健康检查，以及供后续映射原生菜单的 desktop action 模型。Web Admin 新增 `studioNativeBridge` 客户端，运行在 Studio 壳内时自动同步磁盘连接库、展示 Local Server 状态与 Start / Stop / Health 控制，结果导出与关系导入导出优先走原生文件对话框；普通浏览器探测不到 bridge 时继续使用 `localStorage` 连接库、浏览器下载和 `<input type=file>`，实现优雅降级。
- **M29 D #257 文档浏览器接入统一外壳**：Web Admin 新增 `DocumentCollectionWorkbench`，Collections 节点可从统一 Explorer 右键 / 双击进入专用文档工作台；支持集合列表、集合创建 / 删除、Document find 查询（ID / filter / projection / sort / cursor 分页）、count / distinct / aggregate、文档详情查看与 JSONL 导出、单文档 insert / replace / delete、批量选择删除、JSON array / JSONL 导入，以及 JSON path / FullText 索引 rebuild。Validator Governance 接入 schema 只读回显、JSON 编辑、error/warn action、样本文档本地 precheck、保存 / 删除 staged preview。所有写、删、导入、validator 与 rebuild 均复用既有 Document / maintenance API，并统一进入 #247 `WriteApprovalPanel`、共享结果面板和工作台历史；Server schema 仅补充 document collection validator 只读 metadata，不新增文档引擎能力。
- **M29 D #256 对象桶浏览器**：Web Admin 新增 `ObjectBucketWorkbench`，Buckets 节点可从统一 Explorer 右键 / 双击进入专用对象桶工作台；支持桶列表、创建 / 删除、按 prefix + delimiter 浏览对象、分页加载、对象下载、range read 预览（Text / Hex / Base64）、版本列表、presigned URL 生成、对象上传、复制、标签编辑、批量删除，以及 lifecycle / retention / quota / policy / legal hold / audit / 容量统计治理视图。所有创建、删除、上传、复制、标签、生命周期、配额、保留、legal hold、presigned URL 与 multipart 发起 / 分片 / 完成 / 中止均复用既有对象存储 REST API，并统一进入 #247 `WriteApprovalPanel` staged preview、共享结果面板和工作台历史；multipart 会话与分片由服务端持久化枚举，支持跨标签页恢复和继续操作。
- **M34 Modbus TCP 内建映射表规划**：`ROADMAP.md` 新增 Modbus TCP 主站/client 与从站/server 独立路线，规划通过 SQL DDL 定义 `CREATE MODBUS SOURCE`、`CREATE MODBUS ENDPOINT`、`CREATE TABLE ... USING MODBUS ...`，在建表时声明寄存器区域、地址、字段类型、字节序、缩放、读写权限和写入策略；同步调整 M30 边界，将 Modbus 从被动协议接入里程碑拆出，避免主站采集与从站暴露语义混淆。
- **M29 C #255 全文检索 playground**：Web Admin 新增 `FullTextSearchWorkbench`，FullText Indexes 节点可从统一 Explorer 进入专用 BM25 playground；支持查看索引列表、doc/term/tokenizer 统计，执行 exact/fuzzy 检索并展示 Top-K score、命中文档、客户端高亮、分页和历史结果，提供 analyzer token 预览以及 All/Any/Phrase/Fuzzy 查询构建器。Server 管理契约扩展全文统计 term count，并为 `search-preview` 增加 `queryKind=all|any|phrase`，索引 rebuild 继续走 #247 `WriteApprovalPanel` + 既有 `rebuild_index document_fulltext` 维护入口，不新增索引语义或写入路径。
- **M29 C #254 向量检索 playground**：Web Admin 新增 `VectorSearchWorkbench`，Vector Indexes 节点可从统一 Explorer 进入专用 ANN playground；支持查看索引维度、行数、度量、HNSW `m`/`ef`/`efConstruction` 参数，粘贴原始 `float[]` 或通过 Copilot embedding provider 生成查询向量，执行 Top-K 检索并展示 score、tags、fields、过滤条件和历史结果。Server 管理契约扩展向量索引统计与 `search-preview`：按 schema 回显 per-index metric、统计 vector row count，支持只读 metric override、受限 metadata filter（TAG 等值与 time 比较的 AND 组合）并返回命中 metadata；新增 `embed-preview` 只读端点复用既有 Copilot embedding provider，不改变索引语义或写入数据。
...
- **M29 A #247 统一结果面板 + 写审批 / 历史 / 导出框架**：Web Admin 新增跨模型共享 `WorkbenchResultPanel`、`WriteApprovalPanel`、`WorkbenchHistoryDrawer`、`workbenchHistory` store 与 `resultExport`/`sqlWorkbench`/`managementExplorer` 工具模块；`SqlResultPanel` 扩展为 Table / Raw / JSON / Chart 四视图并继续保留 GEOPOINT 地图视图，结果可统一 CSV / JSON 导出。SQL Console 改为消费共享框架，查询和维护操作统一进入历史；原巨型 `SqlConsoleView.vue` 按功能拆分为 Header、Explorer Sidebar、连接/建库弹窗、SQL Query Workspace、共享工具和 `useSql*` composables，给后续 #248~#257 工作台复用。
- **M29 A #246 统一多模型 Explorer + 连接库**：Web Admin 的 SQL Console 左侧 Explorer 升级为统一多模型树，按 Connection → Database → Measurements / Tables / Collections / KV Keyspaces / Index Lifecycle / Vector Indexes / FullText Indexes / MQ Topics / Buckets / Backup 展示对象；新增 #245 管理契约前端客户端，KV、向量、全文、MQ、对象桶 metadata 以只读方式并行加载，单模型端点失败时降级到对应节点提示而不阻断 schema 树。新增可持久化连接库 Pinia store，内置 `Managed Local`，支持添加 Remote 连接与默认数据库，活动连接同步到全局 axios `baseURL`，token 继续只走既有登录会话安全存储、不落入连接库。
...
- **M30 #268 多协议接入收口**：新增 MQTT / Sparkplug B / CoAP / Line Protocol HTTP/UDP 接入矩阵、安全/QoS/可靠性边界与选型指南；`SonnetDB.Parity` 在真实 Server 上以同一 LP payload 对拍 HTTP、UDP、MQTT、CoAP 落库结果，Sparkplug protobuf 保持专用映射测试。UDP listener 改为严格 UTF-8，超限、非法编码和坏 LP 数据报不会终止监听；新增 Sparkplug decode 基准并汇总 vendored CoAP route 基准。
...
- **M33 #285 `count(*)` 专门路径（perf）**：单 field schema 在无 residual / Geo 时复用底层 Count 聚合，完整 block 直接合并 `descriptor.Count`，部分范围与分桶只解码时间戳；Count 的 MemTable、Segment 与 tombstone 逐点回退路径均改为 count-only，String 等非数值字段不再做无意义数值转换。多 field schema 用各字段升序点流的最小堆 k-way merge 计不同时间戳，替代 `Dictionary<long, HashSet<long>>`；residual / Geo 保留原兼容路径。200k 时刻、3 field 的热查询单次分配从 12.0 MiB 降至 43.84 KiB（约 280 倍），Median 基本持平、P90 约下降 10%。完整环境与 Median/P90 见 [M33 基准报告](docs/benchmarks/m33-aggregate-execution-pushdown.md)。
- **M33 #284 残差 / 跨字段 Geo 聚合流式化 + 空残差短路（perf）**：残差数值过滤（`avg(v) WHERE v > k`）、跨字段 Geo 约束（`count(speed) WHERE geo_bbox(position, ...)`）与 `count(*)` 的逐点聚合此前对每 `(series, field)` 先 `QueryPoints(...).ToList()` 把整段点物化为 `List<DataPoint>` 再遍历累加 `AggSlot`，峰值内存 O(N)。而底层 `QueryEngine.Execute(PointQuery)` 本就是单快照租约、单趟 N 路流式合并的惰性 `IEnumerable`——现新增 `QueryPointsStream` helper，`SelectExecutor` 的残差主循环、`BuildGeoAllowedTimestamps` 命中时刻集、`count(*)` 时间戳并集与 `count(<string>)` 等非快路径聚合改为直接惰性枚举、边判残差/Geo 边更新 `AggSlot`，段 block 逐块解码即弃，峰值内存降为 O(1)、CPU 与结果不变；`QueryPoints`（物化版）保留给 raw 路径（时间戳并集 + ts→value 字典需二次遍历）与残差查表构建。另外无残差谓词时 `BuildResidualLookups` 短路返回共享只读空表，消除每 `(series, spec)` 的空字典分配。纯流式聚合路径（`count(<string field>)`：字符串字段绕过数值快路径、count-only 无逐点装箱、无 residual/geo）实测查询期净分配在 10,000 与 40,000 点下均恒为约 5 KB（0.1–0.3 B/点，被移除的 `.ToList()` 原为 ~210 B/点 = `sizeof(DataPoint)`×N），证明不随点数线性增长（GC.GetAllocatedBytesForCurrentThread 预热后测量）。新增 `SqlExecutorAggregateStreamingTests`（4 项：50k 残差聚合对拍、纯流式 count 峰值分配 10k↔40k 增量 < 512KB、30k 跨字段 Geo bbox 对拍、空残差短路 count(*)）。
...
- **M28 P5b #244 全模型接入收口 + 文档 + parity**：在 `tests/SonnetDB.Parity` 新增 `FrameRestTransportParitySuite`，用真实 Kestrel 临时服务在同一数据库内横向验收七个 service（MQ、TSDB、SQL、Vector、KV、Object、Document）的 `Protocol=frame-http2` / 直接二进制帧与 `Protocol=rest` 稳定语义等价：MQ 发布/批发布/拉取/ack payload 与 header 等价，TSDB Line Protocol 写入经列式帧与 REST bulk 回查等价，SQL 只读查询行集等价，Vector 直接 search 帧与 REST SQL KNN 结果等价，KV set/get/scan、Object put/get 内容哈希、Document insert/find JSON 语义等价。`docs/frame-protocol.md` 补 REST vs 帧-HTTP2 选型矩阵、#230/#235/#237~#239 基准报告索引、推送订阅/流式结果集/MQTT 两形态边界与 #244 parity 验收入口；`ROADMAP.md` 将 #244 收口为已完成。

- **M28 P5b #243 MQTT client 订阅外部 broker（接入既有 EMQX/Mosquitto）**：Server 程序集新增外部 MQTT client 后台服务（Core 仍零第三方运行时依赖），配置路径为 `SonnetDBServer:Mqtt:ExternalClient`，支持 `Host`/`Port`/`UseTls`、外部 broker 用户名密码、`ClientId`、clean start、topic filter 列表与指数退避重连。收到的实际 topic 必须匹配 `db/{db}/m/{measurement}`，再复用 #242 抽出的 `SonnetMqttMeasurementIngestor` 与 `BulkIngestEndpointHandler.IngestPayload` 三格式落库逻辑（Line Protocol / JSON points / BulkValues，继续支持 `sndb-format` user property、ContentType 或 payload 自动识别）；无匹配 topic 或不存在数据库只记录并丢弃，不影响订阅循环。外部 client 可在不启用内建 broker 时单独运行，也可与 #242 内建 broker 同时启用；范围明确为拉取外部 broker 的 measurement 数据，不做 broker 桥接、集群、跨节点 session 或外部 `db/{db}/mq/{topic}` 到 SonnetMQ 的桥接。新增 `MqttExternalClientEndpointTests` 覆盖临时外部 MQTT broker → SonnetDB client 订阅 → SQL 回查落库闭环。

- **M28 P5b #242 MQTT 内建 broker（设备直连落库 / 订阅推送）**：Server 程序集新增基于 `MQTTnet.AspNetCore.Routing` 的单机内建 MQTT broker（Core 仍零第三方运行时依赖）。配置新增 `SonnetDBServer:Mqtt`（`Enabled`、`Port`、`WebSocketPath`、`MaxMqSubscriptionsPerClient`），默认 appsettings / 容器配置启用 TCP `1883` 与 WebSocket `/mqtt`，docker-compose 暴露 1883。topic `db/{db}/m/{measurement}` 的 PUBLISH 复用 `BulkIngestEndpointHandler.IngestPayload`，支持 LP / JSON points / BulkValues 三格式，通过 `sndb-format` user property、ContentType 或 payload 自动识别，避免重复落库逻辑；topic `db/{db}/mq/{topic}` 写入 SonnetMQ，SUBSCRIBE 从 latest 起点复用 #236 `WaitForMessagesAsync` 推送管线注入 broker。MQTT 鉴权复用现有用户 / token / 三角色权限模型（username/password 或 token），写入需 Write、订阅需 Read；QoS 0/1 支持、QoS 2 拒绝，精确 topic 不支持通配符，范围明确为单节点内建 broker，不做集群、桥接或跨节点 session。新增 `MqttBrokerEndpointTests` 覆盖 MQTT 入库后 SQL 回查、readonly publish 拒绝、SonnetMQ 订阅推送。

- **M28 P5b #262 对象存储客户端帧贯通**：补 #241 遗漏的**对象存储**客户端出口——远程 `SndbObjectStorageClient` 此前完全没有 `FrameChannel`，`PutObjectAsync` / `OpenReadAsync` 全走 REST + `StreamContent`，服务端 #240 的 `object` service 帧（get 流式 `meta→data×N→end` 分块、put 原始字节零 Base64）在客户端从未接线（N8 客户端侧收口）。`SndbObjectStorageClient` 加 `_frames` 字段（同其余远程客户端）：`Protocol` 允许走帧时，**put**（内容可缓冲进单帧 132 MiB 上限内）经 `ObjectFrameCodec` put 帧原始字节直传——可 seek 的流若超上限则 seek 回原位回落 REST 流式，不可 seek 且超上限抛 `NotSupportedException` 提示改用 multipart；**非 Range 全量 get** 经 object get 流式分块帧（客户端拼接 data 帧为完整内容流并校验 end 帧声明的总字节数）。传输级失败回落 REST（`Protocol=frame-http2` 强制不回落，与 #241 语义一致）；对象/桶不存在时帧回带内 `*_not_found` 错误帧、客户端归一化为 `null`，与 REST 把 404 归一化为 `null` 一致。**恒走 REST**：bucket 管理 / multipart / 大对象（> 帧上限）/ **Range 读** / presigned / tagging / list / head / copy / delete（与服务端 #240 帧 scope 一致，管理面不进帧）。内容字节、`versionId`/`etag`/`sha256`、`forbidden`/`bucket_not_found` 错误码在帧与 REST 两条路径一致；嵌入式路径不变。已知差异（记录在 `docs/frame-protocol.md`）：metadata/tags 值帧路径以 UTF-8 varstring 直传（支持非 ASCII），REST 经 `x-amz-meta-*` 头（仅 ASCII）；get 帧路径回传服务端 meta 帧的 metadata/tags，REST get 不回传这两张字典（S3 兼容语义）。测试：`SonnetDB.Tests` 新增 `ObjectFrameTransportParityTests`（13 项，真实 Kestrel × `Protocol=frame-http2` vs `rest`：put→get 原始字节含高位字节往返、帧写→REST 读 / REST 写→帧读逐字节相等、600 KiB 大 blob 多分块完整、缺失对象两传输均 `null`、缺失桶 PUT 帧路径 `bucket_not_found`、readonly token PUT `forbidden` 两传输同码、Range 读在 frame-http2 下优雅回落 REST 返回 partial content）。所有既有 REST S3 兼容端点保留，帧层不改引擎对象存储语义。

补 #241 遗漏的**时序批量写**客户端出口——远程 `SndbConnection` + `CommandType.TableDirect` 的 bulk ingest 此前恒走 REST `/measurements/{m}/{lp|json|bulk}`，服务端 #237 的 `tsdb` 列式写帧（wire 3.73× 小于 JSON、编码 326× 快）在客户端从未接线（N5 客户端侧收口）。`SonnetDB.Core` 新增 `SonnetDB.Protocol.TsdbColumnarBlockBuilder`（纯 BCL，`TsdbColumnarPointReader` 的逆过程）：把 `Point` 集按 tag 组聚合为 `TsdbColumnarBlock` 列式块——同一组 tag（顺序无关）归一序列族、缺值行用 presence 位图标记稀疏、六种 `FieldType` 全支持，同名字段类型冲突或向量维度不一致抛 `BulkIngestException`。`SonnetDB.Data` 的 `RemoteConnectionImpl` 把 `ExecuteBulk`/`ExecuteBulkAsync` 合流为单一 async 帧优先入口 `ExecuteBulkRequestAsync`：`Protocol` 允许走帧时，**Line Protocol / JSON** payload 经既有 `LineProtocolReader`/`JsonPointsReader` 解析为点集 → `TsdbColumnarBlockBuilder` → `TsdbFrameCodec.EncodeWriteColumnarRequest` 列式写帧优先发送，`?flush` 三档（none/async/sync）映射帧 `flushMode`；传输级失败回落 REST（`Protocol=frame-http2` 强制不回落），解析/列式编码失败（类型冲突、名字超限、超 132 MiB 帧上限）静默回落 REST 让服务端 reader/schema 权威处理并给规范错误。**两条边界恒走 REST**：(1) **BulkValues（`INSERT INTO m(...) VALUES`）**——列的 tag/field 角色需服务端按 measurement schema 解析，客户端无 schema 无法把值分派到 tag 列 vs field 列、无法列式编码；(2) **`onerror=skip`**——帧列式写端点为 FailFast（整帧拒绝），逐行跳过坏行的语义只有 REST 支持。写入行数、`forbidden`/`db_not_found`/`bulk_ingest_error` 错误码在帧与 REST 两条路径同码；嵌入式路径不变。测试：`SonnetDB.Core.Tests` 新增 `TsdbColumnarBlockBuilderTests`（9 项：空点集、单 tag 组稠密块、多 tag 组分块、tag 顺序无关归一、稀疏 presence 位图、六类型往返、同名字段类型冲突/向量维度冲突抛错、不同 tag 组同名字段互不冲突——均经 `TsdbColumnarPointReader` 列转行往返验证语义等价）；`SonnetDB.Tests` 新增 `TsdbBulkFrameTransportParityTests`（15 项，真实 Kestrel × `Protocol=frame-http2` vs `rest`：LP/JSON 写→SQL 回查行数一致、帧与 REST 写入数据逐行相等、BulkValues 与 `onerror=skip` 在 frame-http2 下优雅回落 REST、`?flush=sync` 落库、readonly token `forbidden` 同码、未知库 `db_not_found` 同码、schema 类型冲突 `bulk_ingest_error` 同码）。`docs/frame-protocol.md` 补客户端时序写帧化范围与两条 REST 边界说明；所有既有 REST 批量端点保留，帧层不改引擎写入语义。
...
- **M28 P4 #228 删除遗留 `HnswVectorBlockIndex`（索引 I13）**：删除 `src/SonnetDB.Core/Storage/Segments/HnswVectorBlockIndex.cs`（含文件内 `HnswAnnSearchResult` record）——它是段内 HNSW 图的早期实现，图质量更差、建图为 `O(n·ef²)`、仅支持 cosine，且在 `src/` 内**零生产引用**（生产向量索引路径早已统一走 `VectorIndexAdapter` → `LocalVectorIndexBuilder` → `HnswIndex<int>`，仅一个基准与一个召回测试仍在维护这段死代码，构成误用风险）。仅剩的两个消费者迁移到 canonical `HnswIndex<int>`（`src/SonnetDB.Core/Vector/Index/Hnsw/HnswIndex.cs`）：召回测试重写并重命名为 `HnswIndexRecallTests`（`new HnswIndex<int>(384, Metric.Cosine, {M=16, EfConstruction=200, EfSearch=200})` 建图 + `Search`-into-span，Recall@10 ≥ 0.90 断言不变），`VectorRecallBenchmark` 的 `Hnsw_Top10`/`Hnsw_RecallAt10` 改走同一 API。catalog 生产类型 `HnswVectorIndexOptions`（不在被删文件内）保留不动。

### Changed

- **M28 P4 #226 HNSW 搜索 ef 补偿 tombstone + 重建回收（索引 I6 / I14）**：`HnswIndex<TKey>` 的删除此前仅打 tombstone（节点留在图中保连通、搜索时跳过），带来两个缺陷：(I6) 搜索固定用 `ef = max(EfSearch, topK)` 的候选窗口，过滤掉命中窗口内的 tombstone 后**可能欠返回 topK**——删除越多、候选窗口被死节点挤占越严重，实际返回可能远少于请求的 K；(I14) tombstone-only 删除**从不物理回收内存**，`_entryPoint` 从不重指，churn（反复删增）下 `_vectors`/`_keys`/`_neighbors` 随累计删除无界增长。现两处收口：**搜索侧 ef 补偿**——按 tombstone 占比放大 ef（`ef ≈ baseEf × 总行数 / 存活行数`，上界为物理行数），若一轮搜索后仍未集满 `min(topK, 存活数)` 个存活结果则**倍增 ef 继续搜索**，直到集满或候选窗口覆盖全部物理行（此时等价于精确扫描该连通分量），保证有删除时不欠返回；存活数不足 topK 时返回全部存活且不空转。**删除侧重建回收**——新增 `HnswOptions.AutoCompactTombstoneRatio`（默认 0.2，`[0,1]`，0 表示禁用），`Remove` 后当 `tombstone 数 / 物理行数 ≥` 阈值时**就地重建图**：快照存活行的 (key, vector)、清空全部结构、按启发式邻居选择重新逐条建图并重指 `_entryPoint`，物理丢弃已删除节点、回收内存、重排紧凑 row id（存活行相对顺序不变）；亦提供公开 `Compact()` 显式触发（返回回收的 tombstone 行数）。插入核心逻辑抽为 `AddCore`（要求持写锁）供 `Add` 与重建复用，锁语义（`ReaderWriterLockSlim` 多读单写）不变；持久化快照 / blob 格式（`HnswIndexSnapshot` / SDBVIDX）不变，`AutoCompactTombstoneRatio` 是纯运行时行为不入盘。测试新增：大量删除后搜索仍集满 topK 且只命中存活行、存活数 < topK 时返回全部存活不空转、tombstone 越阈值自动重建（快照观察无 tombstone、行数==存活数）、显式 `Compact()` 回收并保持搜索正确、空索引/无 tombstone 时 `Compact()` 空操作、重建后重插曾删键无冲突、`AutoCompactTombstoneRatio` 越界校验抛错；既有 #193 快照往返测试改为显式关闭自动重建以保留 tombstone 复现重复 key 路径。Core 2658（新增 9）全绿，slnx 0 warnings。

- **M28 P4 #225 compaction/flush/drop 向量索引 catalog 必需（索引 I11）**：含 VECTOR 列的段落盘时，向量索引仅在 `seriesCatalog` 与 `measurementCatalog` **两个 catalog 都提供**时才构建，否则 `vectorIndexes` 为 null，段的向量块被**无索引落盘**、查询时静默退化为暴力扫描——而三条落盘路径中，`SegmentCompactor.Execute` / `FlushCoordinator.Flush`(+`FlushSealed`) 的 catalog 是可选参数（调用方漏传即静默丢索引），`MeasurementDropCompactor.RewriteWithoutSeries` 更用脆弱的 `owner.Catalog.Count>0 && owner.Measurements.Count>0` 作代理判断（catalog 已加载但恰好为空、或未来重构改了这个隐式前提，都会让幸存 measurement 的向量块丢索引）。现把索引解析统一收敛到新的 `VectorIndexBuildMap.BuildForSegment(seriesList, catalog?, measurements?)`：两个 catalog 均非 null 时等价于既有 `Build`；缺 catalog 但待写桶中**存在任一 VECTOR 字段**时**显式抛 `InvalidOperationException`**（把「慢但对」的静默降级转为快速失败的调用契约违反），无向量字段时返回 null。三条路径（flush ×2、compaction、drop-measurement）全部改走此入口，drop 路径删掉 `.Count>0` 代理判断。`SegmentWriter` 仍保留 null-map = 无索引的原始向量块写入能力（存储层裸向量块测试依赖，缺陷唯一能被识别处是 catalog 可见的三个上层调用点）。新增回归：`VectorIndexBuildMapTests`（向量桶缺 catalog / 缺其一 catalog 抛错、无向量桶缺 catalog 返回 null、有 catalog 正常解析声明索引）、`SegmentCompactorTests` 向量段无 catalog 抛错且不留半成品目标段、`SqlExecutorMetadataTests` DROP MEASUREMENT 触发段重写后幸存 measurement 向量块仍带内嵌索引且 KNN 查询正确。Core 全绿，slnx 0 warnings。

- **M28 P4 #224 向量 KNN 用 block skip-index（索引 I8）**：`KnnExecutor` 的段侧扫描此前对每个匹配 series **遍历每个 `SegmentReader` 的全部 Block**（`foreach reader.Blocks`），再逐块按 `SeriesId` / `FieldName` / `FieldType` / 时间窗过滤，代价为 O(段数 × 每段总 block 数)——即便查询窄时间窗只命中极少数块，也要线性扫过与查询无关的其它 series/field/时间段的全部块描述符。现改为经 `MultiSegmentIndex.LookupCandidates(seriesId, field, from, to)` 只召回**与 (series, field, 时间窗) 相交的候选 block**：段级时间剪枝（`SegmentIndex.OverlapsTimeRange`）+ block 级 `MaxTimestamp` 前缀最大值二分（prefix-max skip-index，与 #220 流式合并 / 聚合快路径同款）在索引内完成，块选择从 O(总 block 数) 降为 O(log n + 重叠块数)。`KnnExecutor.Execute` 新增 `MultiSegmentIndex` 参数（由调用方从**同一读租约**的 `Snapshot.Index` 传入，与 `Readers` 同源保证 SegmentId 一致），候选 block 经 `SegmentId → SegmentReader` 映射回读真实 payload。语义与旧逐块路径逐点等价：命中向量索引且**建图度量 == 查询度量**且该 series/field 无墓碑时仍走段内 HNSW ANN 加速（I7 度量 gate、`CollectIndexedBlockCandidates` 去重、有墓碑放弃 ANN 走精确扫描全部保留），否则精确扫描；MemTable 扫描、Top-K 堆、墓碑过滤均不变。两个调用点 `TableValuedFunctionExecutor`（`knn(...)` TVF 与帧协议向量检索共用的 `ExecuteKnnSearch`）与 `HybridSearchExecutor` 同步传入 `Snapshot.Index`。新增 5 项端到端回归：多段不相交时间带 + 窄窗口只召回命中段、多段全窗口跨段 Top-K 全局正确、同段多 series tag 过滤只命中匹配 series、时间窗落在所有段之外返回空、MemTable + 多已刷盘段合并。Core 2645（新增 5）全绿。
