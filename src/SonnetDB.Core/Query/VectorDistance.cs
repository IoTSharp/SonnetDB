...
internal static class VectorDistance
{
    ...
    public static double ComputeNegativeInnerProduct(ReadOnlySpan<float> a, ReadOnlySpan<float> b)
        => DotVector.Primitives.VectorDistance.ComputeNegativeInnerProduct(a, b);

    private static DotVector.Primitives.KnnMetric ToDotVectorMetric(KnnMetric metric)
        => metric switch
        {
            KnnMetric.L2 => DotVector.Primitives.KnnMetric.L2,
            KnnMetric.InnerProduct => DotVector.Primitives.KnnMetric.InnerProduct,
            _ => DotVector.Primitives.KnnMetric.Cosine,
        };
}
