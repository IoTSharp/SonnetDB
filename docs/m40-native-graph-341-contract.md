...
| `M40-GAP-009` | statement snapshot、supernode/维护、高级算法准入、运维面和生产证据尚未全部交付 | SonnetDB M40 #360～#367 | Production、Couplet C4 | `in_progress`；#360 已以 `GraphStatementSnapshotTests`、原生 `EXPLAIN ANALYZE` snapshot sequence 和 [#360 合同](m40-graph-360-statement-snapshot.md)关闭功能切片；#361 的有界分页、state v5 压缩、可恢复维护、流式 degree statistics 和 100k supernode 回归已关闭功能切片；#362 的加权 Dijkstra/A*/双向 Dijkstra、批量执行、预算/错误合同与 HTTP/SDK correctness smoke 已接线，但真实 journey 和收益 benchmark 仍 open。#352/#367 固定硬件/7 天证据、#363～#366 及 Couplet C4 联合证据仍 open，不能提升产品定位 |
