...
3. 在管理员账号下新建一个数据库，确认列表刷新且新数据库可被选中。
4. 删除一个非系统数据库，确认有二次确认提示且删除后树会刷新。
5. 输入 `SELECT ...`，点击运行，确认结果直接进入 Result Grid。
6. 输入 `EXPLAIN SELECT ...`，确认 Result Grid 显示 `key` / `value` 估算行。
7. 输入 `INSERT` 或 `CREATE MEASUREMENT`，确认先出现 Staged Preview，而不是直接写入。
8. 输入 `DROP` / `DELETE` / `GRANT` / `REVOKE` / `CREATE USER`，确认出现危险操作提示和确认勾选。
9. 切换到 Trajectory 模式，确认轨迹地图仍能在 Workbench 内加载并回放。
10. 切换数据库后，确认预览状态失效并需要重新生成。
11. 打开右下角 CopilotDock，确认它仍是全局浮窗。
