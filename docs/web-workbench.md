...
Studio 沿用 Vue + CodeMirror + Naive UI 技术栈，并复用服务端已经存在的数据库列表、schema、SQL 执行、事件和 Copilot stream 协议。它不要求额外部署 BI 或数据库管理工具，启动 SonnetDB Server 后即可使用。桌面客户端通过 `SonnetDB.Studio` 使用 NativeWebHost 打开同一套 Studio 入口。八模型专用工作台、桌面增量能力和 VS Code 覆盖边界见[管理工具与三面能力矩阵]({{ site.docs_baseurl | default: '/help' }}/management-tools/)。
...
- **Schema Explorer**：展示数据库树、measurement、关系表、文档集合、KV keyspace、向量/全文索引、MQ topic、对象桶、索引生命周期和备份状态；管理员可创建或删除数据库。
