...
description: "SonnetDB Web Admin Workbench 的产品能力说明，覆盖 Schema Explorer、SQL Editor、结果视图、轨迹地图、CopilotDock、写入审批和手工验收。"
permalink: /web-workbench/
---

# SonnetDB Workbench

SonnetDB Workbench 是 Web Admin 里的核心工作界面，路径为 `/admin/app/sql`。它把数据库浏览、SQL 编辑、写入预览、结果分析、轨迹地图和 Copilot 智能协作放在同一个操作面里，让 SonnetDB 不只是一个后端引擎，也是一套可以直接交付给开发者、运维人员和数据使用者的管理工作台。

Workbench 沿用 Vue + CodeMirror + Naive UI 技术栈，并复用服务端已经存在的数据库列表、schema、SQL 执行、事件和 Copilot stream 协议。它不要求额外部署 BI 或数据库管理工具，启动 SonnetDB Server 后即可使用。
...
- **CopilotDock**：全局浮窗，可在 Workbench、事件、权限、设置等页面共享会话和上下文。

## Copilot 协作

Workbench 中的 Copilot 不是孤立聊天框，而是伴随式数据库智能体：
...
- 写入 SQL 会同步到 Workbench tab 或 staged preview，避免无感修改数据。
