...
VictoriaMetrics 就绪修复后的五项受控 HTTP 回归通过，覆盖部分可见、多个 measurement、缺点超时、超额点拒绝和调用方取消。固定 v1.106.1 真实服务的六个支持场景均通过、无结果差异，写入自检读回 10,000/10,000 点，计时包含可见性等待；Holt-Winters 仍按能力缺口跳过。旧适配器的独立重复探针在本机完成十六轮成功，第十七轮被原定三分钟总预算中止，因此仍不宣称复现了线上失败，也不将二十轮计划算作全部通过。新增实际点数指标和十五秒完整数量就绪检查保留严格判定，随后用新的统一提交重跑完整 Actions。
