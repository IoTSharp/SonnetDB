...
后续更新：按用户指定，#335/#336 的实现、同语义对照和验收已单独登记在 [高级触发器验收](m39-335-336-20260906.md)。本文件后面的“暂未准入”记录保留本轮早期核查时点，不代表后续交付状态。
...
M39 编码与本地验收已完成。性能证据尚未覆盖生产混合负载和长期 SLO；按项目治理规则，这些设备相关验证转入仓库 ROADMAP 的[真机验证待办](../../ROADMAP.md#真机验证待办)，不阻塞 M39 研发完成状态。
...
| #335 | 已完成编码与本地准入 | 真实项目服务器上的批量负载与尾延迟进入真机验证待办 |
| #336 | 已完成编码与本地准入 | 真实项目服务器上的归一化负载与尾延迟进入真机验证待办 |
| #337 | 定义校验、过滤审计、窗口分布及快照导出本地通过 | 完整持久审计或订阅有独立需求时另立合同，不把快照冒充无损日志 |
| #338 | 已完成编码与本地准入 | 真实项目服务器上的 deferred/outbox 混合负载与外部消息故障进入真机验证待办 |
| #339 | 已完成编码与本地准入 | Document/measurement 原生事件、幂等/重放、保留/compaction、备份与容量证据已具备；真实项目服务器上的固定硬件与长期 SLO 进入后续真机验证 |
