...
1. `RemoteConnectionImpl.BeginTransaction` (`src/SonnetDB.Data/Remote/RemoteConnectionImpl.cs:579`) creates only a client-side `RemoteTransactionState`; no server transaction is opened.
2. A transactional `ExecuteReader[Async]` with `INSERT ... RETURNING` enters `ExecuteTransactionalInsertReturningRequestAsync` (`RemoteConnectionImpl.cs:748`). `DO UPDATE` is rejected before a schema request, preview, or queue append (`:753`). Other returning inserts preview with a separate `/sql/batch` request containing `BEGIN`, prior queued SQL, the target SQL, and `ROLLBACK` (`:914`).
3. `CommitTransactionAsync` (`RemoteConnectionImpl.cs:597`) sends a new `/sql/batch` request containing `BEGIN`, queued SQL, and `COMMIT`. The server's `SqlEndpointHandler` holds its `SqlTransactionContext` only inside one request (`src/SonnetDB/Endpoints/Handlers/SqlEndpointHandler.cs:226`) and rolls back an unfinished transaction when that request ends (`:495`). There is no transaction identifier or endpoint that resumes that context.
