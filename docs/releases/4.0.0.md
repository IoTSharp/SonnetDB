...
Publish workflow 在 Windows/Linux 两个平台生成并验证 NativeAOT Server，所有协议检查通过后才推送同一批 NuGet 包并创建 GitHub Release。`workflow_dispatch` 可执行候选验证，不推送包。各次 `result.json`、消费者及 Server 日志保存在 `insert-returning-contract-<rid>` artifact。候选本机证据见 [#197 包验收记录](../audits/issue-197-package-contract-20260926.md)。
