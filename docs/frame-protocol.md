...
- **边界**：#242 只实现单节点内建 broker，不做 broker 集群、桥接或跨节点 session；外部 broker 拉取由
  #243 的 MQTT client 形态负责。

## MQTT client 订阅外部 broker（M28 P5b #243）

SonnetDB Server 也可作为 MQTT client 主动连接既有 EMQX / Mosquitto 等 broker，把外部 broker 上的
measurement topic 拉入 SonnetDB。该能力与内建 broker 相互独立：可以只启用外部 client，也可以同时启用
内建 broker 与外部 client。
...
"SonnetDBServer": {
  "Mqtt": {
    "ExternalClient": {
      ...
      "Port": 1883,
      "UseTls": false,
      "ClientId": "sonnetdb-external-client",
      "UserName": "collector",
...
- **订阅**：`Subscriptions[].TopicFilter` 是发给外部 broker 的 MQTT topic filter，可使用 `+` / `#`。
  但收到的实际 topic 必须是无通配符的 `db/{db}/m/{measurement}`，否则只记录日志并丢弃。
- **落库**：实际 topic 解析出 database + measurement 后，复用 #242 的 `SonnetMqttMeasurementIngestor`
  与 HTTP bulk ingest 同一处理器，支持 Line Protocol、JSON points 与 BulkValues。格式识别规则同内建 broker：
  MQTT user property `sndb-format`、`ContentType` 或 payload 首字节自动识别。
- **重连**：连接失败或断开后按 `ReconnectDelaySeconds` 起步指数退避，封顶 `MaxReconnectDelaySeconds`。
- **QoS**：订阅 QoS 支持 0/1；QoS 2 不在本阶段范围内。
- **权限边界**：外部 client 是服务端显式配置的受管采集器，外部 broker 的用户名/密码只用于连接上游 broker，
  不映射为 SonnetDB 用户。请通过 topic filter 与外部 broker ACL 限定可写入的数据库/measurement 范围。
- **范围**：#243 只拉取 measurement 数据；不做 broker 桥接、跨 broker session、集群转发，也不把外部
  `db/{db}/mq/{topic}` 桥接到 SonnetMQ。

## Line Protocol UDP 监听（M30 #267）

SonnetDB Server 可启用一个 InfluxDB 兼容的 Line Protocol UDP listener，供 Telegraf `influxdb` UDP output
或受约束设备以 fire-and-forget 方式上报遥测。该能力只存在于 Server 层，Core 继续保持零协议运行时依赖。
...
- **监听**：UDP 默认端口 `8089`，默认关闭；启用后每个 UDP 数据报视为一批 Line Protocol 行。
- **目标数据库**：UDP 没有 query 参数，目标数据库由 `Database` 配置绑定；measurement 仍从每行 LP 文本解析。
- **落库路径**：服务端复用 `LineProtocolReader` + `BulkIngestor`，与 HTTP `/write` 的 LP 解析和写入语义一致。
- **时间精度**：`Precision` 支持 `n/ns`、`u/us/µs`、`ms`、`s`，默认 `ns`。
- **安全边界**：UDP 入口无鉴权、无 ack、无应用层背压，错误只记录日志并丢弃数据报；必须只暴露在可信内网或本机采集链路。
- **范围**：#267 不实现 UDP 响应、重传、TLS/DTLS、批确认或跨数据库动态路由；需要权限、错误响应和背压时使用 HTTP `/write`、REST bulk、MQTT 或 CoAP/coaps。
...
| 可信内网内的低开销遥测上报 | Line Protocol UDP listener | 数据报即 LP 批次，默认关闭；无鉴权、无 ack、无背压，适合可丢包采集链路 |
...
按 tag 组聚合为列式块（缺值行用 presence 位图标记稀疏），经 `TsdbFrameCodec` write-columnar 帧（service=2, op=1）发送，
`?flush` 三档映射为帧 `flushMode`。传输级失败回落 REST（`Protocol=frame-http2` 强制不回落）；解析/列式编码失败
（类型冲突、名字超限、超帧上限）静默回落 REST 让服务端 reader/schema 权威处理。两条边界恒走 REST：

- **BulkValues（`INSERT INTO m(...) VALUES ...`）**：列的 tag/field 角色需服务端按 measurement schema 解析，
  客户端无 schema 无法把值分派到 tag vs field 列 → 无法列式编码；
- **`onerror=skip`**：帧列式写端点为 FailFast（整帧拒绝），逐行跳过坏行的语义只有 REST 支持。

> 注：缺省时间戳（LP/JSON 未给 timestamp）在帧路径由**客户端**盖章（reader 解析时取 `UtcNow`），
> REST 路径由**服务端**盖章。二者差一个网络往返的时钟差；需服务端权威时间戳请在 payload 显式给出 timestamp。

写入行数、`forbidden` / `db_not_found` / `bulk_ingest_error` 错误码在帧与 REST 两条路径同码；数据落库与 REST 字节等价。

**对象存储客户端帧化（#262）**：远程 `SndbObjectStorageClient` 的 `PutObjectAsync` / `OpenReadAsync` 在 `Protocol`
允许走帧时优先经 object service（service=6）帧，内容原始字节零 Base64：

- **put**：内容缓冲成字节后经 `ObjectFrameCodec` put 帧（单帧）发送。仅当内容能缓冲进单帧上限内才走帧——
  可 seek 的流若长度超上限，seek 回原位回落 REST 流式；不可 seek 且超上限则抛 `NotSupportedException`
  提示改用 multipart。传输级失败复用已缓冲字节回落 REST。
- **get**：仅**非 Range 全量读**走帧（`meta → data × N → end` 流式分块，服务端逐块 256 KiB flush），
  客户端拼接 data 帧为完整内容流并校验 end 帧声明的总字节数。**Range 读恒走 REST**（无帧 op）。
  对象/桶不存在时帧回带内 `*_not_found` 错误帧，客户端归一化为 `null`——与 REST 把 404 归一化为 `null` 语义一致。
- **恒走 REST**：bucket 管理、multipart、大对象（> 帧上限）、Range 读、presigned、tagging、list/head/copy/delete。

内容字节、`versionId` / `etag` / `sha256`、`forbidden` / `bucket_not_found` 错误码在帧与 REST 两条路径一致。
> 注：metadata / tags 值在帧路径以 UTF-8 varstring 直传（支持非 ASCII），REST 路径经 `x-amz-meta-*` HTTP 头
...
- tsdb 行→列聚合器（#261，客户端 SDK 写帧编码）：`src/SonnetDB.Core/Protocol/TsdbColumnarBlockBuilder.cs`
  （`Point` 集按 tag 组聚合为 `TsdbColumnarBlock`，缺值行 presence 位图标记稀疏，`TsdbColumnarPointReader` 的逆过程）；
  客户端写帧优先分发在 `src/SonnetDB.Data/Remote/RemoteConnectionImpl.cs`（`ExecuteBulkRequestAsync` → `TryBuildColumnarFrame`）
- 对象存储客户端帧化（#262，客户端 SDK put/get 帧贯通）：`src/SonnetDB.Data/ObjectStorage/SndbObjectStorageClient.cs`
  （`PutObjectAsync` → `TryBufferForFrameAsync` + `ObjectFrameCodec` put；`OpenReadAsync` → `TryOpenReadFrameAsync`
  拼接 meta/data/end 流式帧；传输级失败与 Range 读回落 `PutObjectRestAsync`/`OpenReadRestAsync`）
...
- MQTT 接入（#242/#243）：`src/SonnetDB/Mqtt/`（内建 broker route controller、SonnetMQ 推送桥、
  外部 broker client 后台服务与共享 measurement 落库服务）
