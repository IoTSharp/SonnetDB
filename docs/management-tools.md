...
本文矩阵以 2026-07-12 的仓库实现为准。`完整` 表示当前交付面有专用入口；`部分` 表示可通过 SQL、共享结果面板或有限维护动作完成，但没有该模型的完整专用流程；`不支持` 表示当前没有可交付入口。对标产品用于确定交互方向，不表示 wire protocol、集群能力或全部功能等价。
...
当前 CI 会生成开发验证用 VSIX artifact，但完整 VS Code Extension Host / Electron workbench UI 自动化、正式打包发布和 Marketplace 截图仍归 M18 #108；当前 #260 不把 HTTP consumer smoke 或开发 VSIX 描述成实机 UI e2e / 正式发布验收。

`.github/workflows/management-workbench-smoke.yml` 会在管理面、共享契约或桌面宿主相关路径变化时运行 8 条 Server 契约 e2e、Web production build、43 条 Chromium e2e、Windows Studio Release build、9 条 VS Code consumer / language-helper smoke，并生成开发验证用 VSIX。Server TRX 与 Web 失败 trace 会上传为 workflow artifact。
