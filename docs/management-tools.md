...
| --- | --- | --- | --- | --- | --- | --- |
| 时序 measurement | Measurement Workbench + SQL + Trajectory | 点编辑、文件导入、目标监控、schema、图表、轨迹 | InfluxDB Explorer / Grafana Explore | 完整 | 完整 | 部分 |
| 关系表 | Relational Table Workbench | 数据网格、行编辑、EXPLAIN、设计器、索引、ER、导入导出 | pgAdmin / SSMS / Navicat / DBeaver | 完整 | 完整 | 部分 |
| 文档集合 | Document Collection Workbench | 查询构建器、CRUD、局部更新预览、索引设计、Change Feed、validator、JSONL | MongoDB Compass | 完整 | 完整 | 部分 |
...
| --- | --- | --- | --- | --- | --- |
| 时序 | 完整：measurement/列/索引 | 完整：时间窗/TAG 过滤、SQL、图表、Trajectory | 完整：点新增/校正/删除，统一 preview；校正需改变 time/TAG 身份 | 完整：CSV/JSON/JSONL 导入，CSV/JSON 导出 | 完整：单 Measurement 实时趋势与最近结果 |
| 关系 | 完整：表、列、主外键、索引 | 完整：分页/过滤/排序、SQL、EXPLAIN | 完整：行编辑、表设计、索引维护，统一 preview | 完整：CSV/JSON/JSONL、DDL | 完整：单表实时最近结果与查询耗时 |
| 文档 | 完整：集合、索引、validator | 完整：可视化 filter、find/count/distinct/aggregate | 完整：CRUD、服务端局部更新预览、compound/unique/sparse/partial/TTL 索引设计与共享审批 | 完整：JSON/JSONL，100 条分批进度与停止后续批次 | 完整：操作历史、索引一致性与可续传 Change Feed Viewer |
| KV | 完整：keyspace、prefix、TTL、值视图 | 完整：scan/get-many | 完整：set/remove/expire/persist/prefix delete，统一 preview | 完整：Base64 值与逐 key expiry 的 JSONL 文件 round-trip、批量文本和结果导出 | 部分：key/expiry 统计 |
| SonnetMQ | 完整：topic、offset、header、payload | 完整：offset browse、时间定位 | 完整：publish/ack，统一 preview | 完整：JSON/JSONL/NDJSON 消息文件导入，支持 topic/header/Base64 或 JSON payload | 完整：lag、吞吐、backlog、retention、DLQ 提示 |
...
[从 Visual Studio Marketplace 安装 SonnetDB for VS Code](https://marketplace.visualstudio.com/items?itemName=iotsharp.sonnetdb-vscode)，或在终端运行：
...
| 数据模型 | 浏览 | 查询 | 编辑 | 导入导出 | 监控 |
| --- | --- | --- | --- | --- | --- |
| 时序 | 完整：measurement/列 | 完整：SQL + Table/Raw/Chart，GEO 可用 Trajectory | 不支持：不计入任意 SQL 写入能力 | 不支持 | 不支持 |
| 关系 | 完整：表/列 | 完整：SQL + Table/Raw/Chart | 不支持 | 不支持 | 不支持 |
| 文档 | 完整：集合/index metadata | 部分：专用只读 find 面板与 SQL | 不支持 | 不支持 | 不支持 |
| KV | 完整：keyspace/entry preview | 完整：只读 scan preview | 不支持 | 不支持 | 不支持 |
| SonnetMQ | 完整：topic/message preview | 完整：只读 browse | 不支持 | 不支持 | 完整：运行监控快照展示 lag、retention 与 DLQ 状态 |
...
| 结果面板 | Table / Raw / JSON / Chart，GEO 可用 Map | 与 Web Admin 相同，保存可走原生对话框 | Table / Raw / Chart，GEO 可用 Trajectory |
...
首次配置 Playwright 的开发机或 CI runner 需要准备 Chromium；已预装浏览器的开发机不应在日常回归中重复下载。

Server 使用真实 Kestrel、临时数据库目录和 admin / readonly token 执行 8 条契约端到端测试，覆盖 KV、向量、全文、MQ、游标分页、输入拒绝和未认证访问。指定独立 artifacts 目录可避免本机已运行的 SonnetDB 锁住默认构建输出：

```powershell
dotnet test tests/SonnetDB.Tests/SonnetDB.Tests.csproj `
  --configuration Release `
  --artifacts-path artifacts/m29-server `
  --filter FullyQualifiedName~ManagementContractEndpointTests
```

Studio 桌面宿主在 Windows Release 配置下单独编译，确保 NativeWebHost bridge、source-generated JSON 和现有 Win32 原生菜单基础映射持续可构建：

```powershell
dotnet build src/SonnetDB.Studio/SonnetDB.Studio.csproj `
  --configuration Release `
  --artifacts-path artifacts/m29-studio
```

VS Code 扩展使用 loopback HTTP server 对 `SonnetDbClient` 做消费端 smoke，覆盖 schema、SQL NDJSON、KV、向量、全文、MQ 监控和对象桶；另用真实 VS Code 验证 Extension Host：
...
当前 CI 会生成开发验证用 VSIX artifact，并在 Windows 隔离 user-data/extension 目录中运行真实 VS Code Extension Host smoke，覆盖扩展激活、命令、diagnostics、signature help 与 quick fix。Marketplace 安装入口已经公开；后续版本更新继续执行完整测试、隔离安装、明确授权和远端哈希核对。

`.github/workflows/management-workbench-smoke.yml` 会在管理面、共享契约或桌面宿主相关路径变化时运行 8 条 Server 契约 e2e、Web production build、43 条 Chromium e2e、Windows Studio Release build、12 条 VS Code consumer / language-helper smoke、真实 Extension Host smoke，并生成开发验证用 VSIX。Server TRX 与 Web 失败 trace 会上传为 workflow artifact。
...
| #259 多模型消费 | `sonnetdbClient.ts`、`sonnetdbTreeDataProvider.ts`、`extension.ts` | KV/向量/全文/MQ 只读浏览与预览已接线；`0.3.0` 继续补齐对象桶与实例/MQ 监控快照 |
| #259 结果与 Copilot | `queryResultPanel.ts`、`copilotPanel.ts` | Table/Raw/Chart、GEOPOINT Trajectory 与 streaming Copilot 已实现；完整编辑仍不属于 VS Code 定位 |
