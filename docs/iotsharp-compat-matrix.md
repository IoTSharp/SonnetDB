...
范围：梳理 `external/IoTSharp` 当前数据底座能力，并为 SonnetDB 替代关系库、时序库、缓存、对象桶、向量搜索与全文搜索建立 #109 基线验收清单。
...
| SQLite | `DataBase=Sqlite`，`ConfigureSqlite` | 本地体验、安装向导、轻量部署 | 并发写、大小写搜索、迁移切换 | SonnetDB 单栈 profile 应至少达到 SQLite 本地体验易用性。 |
| Oracle | `DataBase=Oracle`，`ConfigureOracle` | EF Core、迁移、健康检查 | 标识符长度、序列、分页、大小写 | provider 需记录不支持清单，不以 Oracle 完整替代作为首批门槛。 |
...
| SQLite | `CachingUseIn=SQlite` 枚举存在 | 当前启动逻辑未显式注册 SQLite provider | 入口不完整 | 作为不支持项记录，不纳入首批替代目标。 |
| SonnetDB | 当前未有 `CachingUseIn=SonnetDB` | 规划 EasyCaching / `IDistributedCache` provider | TTL 与并发语义未落地 | #116 前不得宣称可替代 Redis/LiteDB/InMemory。 |
...
- 在 #116/#117/#119 完成前，不宣称 SonnetDB 已完整替代 IoTSharp 的 Redis、S3 或关系数据库。
