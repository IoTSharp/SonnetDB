...
- #110 已完成 ADO.NET 轻事务、异步 API、取消令牌与远程 `/sql/batch` 事务基线；单表轻事务可提交/回滚，跨表事务继续明确拒绝。
- #111 已启动 `DbDataReader.GetSchemaTable()` schema metadata 基线，后续继续补 DDL、`INFORMATION_SCHEMA` 与 provider manifest metadata。
