...
`DrainAsync` 会停止新的轮询，已经预取的消息仍会在枚举器继续读取时交付。取消枚举会停止当前读取；未确认消息不会被自动确认。手动模式下可调用 `delivery.NackAsync(reason)` 请求重投；服务端按 `MaxDeliveryAttempts` 达到上限后把消息转入 `<topic>.dlq`，并在消息头保留原 Topic、offset、投递次数和拒绝原因。需要运维回放时，可通过共享客户端 `ResetOffsetAsync` 选择 earliest、latest、UTC ticks 或 explicit offset；目标会裁剪到 retention 边界。该合同仍不提供 exactly-once 或跨节点 rebalance 承诺，也不提供 message-id 去重窗口。
