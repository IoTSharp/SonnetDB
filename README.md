...
SonnetDB 是一个面向 IoT、工业数据、可观测性、边缘计算和应用后端的一体化嵌入式数据底座。它不再只是时序数据库，而是在同一套 C# / .NET 引擎、同一份本地持久化目录和统一的 SQL / API 访问面中，集成了多种常用数据能力：

- 📈 **时序数据库**：面向设备指标、工业采集、日志指标与实时分析，支持 measurement / tag / field / time 建模、时间窗口聚合、压缩、WAL、Segment、Compaction 与 Retention。
- 🧾 **关系型数据库**：提供 `CREATE TABLE`、主键、索引、约束、事务、JOIN、子查询、聚合与 ADO.NET / EF Core 生态接入，适合存储业务维表、配置表和应用数据。
- 🧩 **KV NoSQL**：内置轻量 keyspace，支持前缀扫描、TTL、批量读写和缓存 Provider，可作为设备状态、会话、配置、缓存等低延迟键值存储。
- 🪣 **S3 兼容存储桶**：提供 bucket / object metadata、分片上传、Range 读取、Presigned URL、对象标签与删除标记，可把文件、图片、附件、备份片段等对象数据直接放进 SonnetDB 管理。
- 📬 **本地消息队列**：内置 SonnetMQ 能力，提供 topic publish、consumer group pull / ack 和重启 replay，适合边缘侧事件缓冲、任务分发和系统内异步解耦。

这些能力可以以进程内嵌入式引擎、独立 HTTP 服务端、Web Admin、CLI、ADO.NET / EF Core、多语言连接器（C、Go、Rust、Java、Python、VB6、PureBasic）等方式使用。
...
- **时序数据引擎**：支持 measurement / tag / field / time 建模，WAL、MemTable、Segment、Compaction 与 Retention 持久化链路。
- **SQL 数据面**：支持 `CREATE MEASUREMENT`、`INSERT`、`SELECT`、`DELETE`、聚合、时间窗口、关系表、JOIN、子查询和 `EXPLAIN`。
- **分析函数**：内置统计聚合、分位数、窗口差分/速率/平滑、预测、异常检测、PID 控制、地理空间与轨迹分析函数。
- **多模型扩展**：提供 KV、关系表、JSON 文档、全文检索、向量检索、Hybrid Search、对象桶和本地消息队列基础能力。
- **访问方式**：提供嵌入式 API、ADO.NET、CLI、HTTP API、Docker 镜像、Web Admin Workbench 和多语言连接器。
- **服务端控制面**：内置首次安装、用户、授权、Token、事件流、健康检查、备份恢复与维护接口。
...
Web Admin Workbench（Schema Explorer / SQL Editor / Staged Preview / Result Grid / Trajectory 模式）说明与验收清单见 [docs/web-workbench.md](docs/web-workbench.md)。
...
- ADO.NET：NuGet 包 `SonnetDB`，命名空间 `SonnetDB.Data`
- EF Core：NuGet 包 `SonnetDB.EntityFrameworkCore`，扩展方法 `UseSonnetDB(...)`
- CLI：`SonnetDB.Cli`
- 远程 HTTP：`SonnetDB`
