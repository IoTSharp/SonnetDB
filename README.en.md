...
### Fourteen Capabilities and Maturity

Beyond the nine native data models, SonnetDB tracks spatial/trajectory, stream processing/subscriptions, CDC/edge synchronization, multimodal AI/RAG, and governance/recovery/observability as five platform capabilities, for fourteen capabilities in total. The [machine-readable evidence index](docs/audits/fourteen-capability-evidence-index.json) records stable IDs, real entry points, roadmap ownership, evidence paths, and boundaries. Public status is limited to `supported`, `partial`, `planned`, `not_planned`, and `beta`; see [Capability Maturity](docs/capability-maturity.md) for the contract. `beta` means a bounded contract is available, not that production gates have passed.
