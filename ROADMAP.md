...
| 41 | 关系查询规划与执行性能加固 | 🚧 | #368 性能合同、#369~#372 快速路径与关系输入下推已完成本地实现和自动化门禁；固定硬件和生产门禁保持 `NOT_RUN`，继续按快照锁范围、流式执行、统计成本模型、高级 JOIN/spill/并行推进。 |
| MM9 | 多模型备份恢复第一批 | ✅ | `BackupService` 与 `sndb backup` 已落地。 |

## 当前推进顺序

1. M41 P0/P1 作为生产稳定性最高优先级：#368 基线与可观测性、#369~#371 的 `EXISTS/IN`、索引并集和倒序 Top-N，以及 #372 的关系输入谓词/投影/安全 LIMIT 下推已完成本地实现、差分回归与 BenchmarkDotNet 对拍接线；木垒同语料、固定硬件和生产 gate 仍为 `NOT_RUN`。下一步按既定顺序推进 #374，不得用增加 SQL permit、内存或索引数量代替根因修复。
...
#369~#372 当前仅完成本地自动化门禁：固定随机种子差分覆盖主键/二级索引 semijoin、索引 OR 与有符号倒序窗口，事务写集验证安全回退；#372 覆盖双侧索引谓词、跨输入残余、LEFT JOIN NULL 语义、聚合、相关子查询、逻辑视图、事务 overlay、有状态 UDF 回退与无排序纯 LEFT JOIN 的安全输入窗口。BenchmarkDotNet 使用值相同的未索引镜像列作为 P0 关系扫描/全扫 Top-N 参考，并以逻辑等价的跨输入残余作为 #372 JOIN 后过滤参考；本地 30,000 任务/128 设备对拍均值由 128.44 ms 降至 61.61 ms，托管分配由 30.18 MB 降至 17.94 MB。木垒生产同语料只读复测、固定 ARM64/x64 硬件数字和生产发布 gate 均保持 `NOT_RUN`，不以开发机数字冒充发布证据。
