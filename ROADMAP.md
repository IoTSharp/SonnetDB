...
| 15 | 地理空间类型与轨迹分析 | #70 ~ #77 | ✅ |
| 16 | Copilot 产品化升级（嵌入式 AI 助手 UX） | #78 ~ #88 | ✅ |
| 17 | 可观测性与运行时可见性（OTel + 结构化日志 + 诊断端点） | #89 ~ #98 | 📋（**#89~#91 前置于 M28 P5b #235**，2026-07-04 决策；#92~#98 不阻塞随后推进） |
...
| 28 | 可靠性、并发正确性与热路径加固（P0~P5 分阶段） | #189 ~ #244 | 🚧（P0~P3 + P5a ✅；**SonnetMQ 硬化 #230→#234 收官，数据面 🔴 全部关闭（含 MQ3 无界内存/OOM）**；下一步 = P4 索引/向量 或 P5b 全模型帧接入 + MQTT，可按带宽并行，**P5b #235 前先落 M17 #89~#91 可观测性基线**；审计 54 项 + P5 新增 MQ/N 专项） |
