...
| 29 | 多模型统一管理工作台（Multi-Model Management Workbench） | #245 ~ #260 | 🚧（#245~#251 ✅；Web Admin 旗舰优先） |
...
| 33 | 时序聚合执行与下推优化（aggregate execution & pushdown） | #282 ~ #287 | 进行中（#282 跨字段 Geo 正确性止血 ✅；#283 同字段多聚合单次扫描 ✅；#284~#287 执行/下推优化待做） |
...
| KV / MQ / 向量 / 全文 专用工作台 | Web Admin | M29 #251~#255 | 🚧（#251 ✅） |
...
| #284 | **残差聚合流式化 + 空 lookup 短路**：新增 `QueryPointsStream(...)` helper，残差路径（1357 行）与跨字段 Geo 的 `.Where().ToList()`（1213 行）改惰性枚举，边判残差/Geo 边更新 `AggSlot`，内存 O(N)→O(1)、CPU 不变；顺带 `where.Residual is null` 时短路 `BuildResidualLookups` 的 per-series 空字典分配（1312 行）。 | 📋 |
