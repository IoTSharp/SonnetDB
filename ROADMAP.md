...
| N2 | 🟠 | `Endpoints/Routes/MessageQueueEndpoints.cs` | 无 HTTP/2 二进制端点，无多路复用 / 流式帧 | #235 ✅（服务端；客户端 SDK 贯通归 #241） |
...
| 28 | 可靠性、并发正确性与热路径加固（P0~P5 分阶段） | #189 ~ #244 | 🚧（P0~P3 + P5a ✅；**SonnetMQ 硬化 #230→#234 收官，数据面 🔴 全部关闭（含 MQ3 无界内存/OOM）**；**M17 #89~#91 可观测性基线已落地**；**P5b #235 通用二进制帧协议 ✅（帧信封 + MQ 四 opcode + h2c 5081 + 编码基准，#236~#240 挂载底座就位）**；下一步 = P5b #236 流式推送订阅 或 P4 索引/向量，可按带宽并行；审计 54 项 + P5 新增 MQ/N 专项） |
