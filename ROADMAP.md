...
| 41 | 关系查询规划与执行性能加固 | 🚧 | #368~#380（含流式算子、covering/index-only、KV/Table 快照、持久统计、成本/JOIN 优化、阻塞算子 spill、受控并行与运行时反馈）已完成本地实现和自动化门禁；固定硬件、木垒生产同语料、7 天 mixed workload 与发布门禁保持 `NOT_RUN`，下一步推进 #381 生产收口。 |
| MM9 | 多模型备份恢复第一批 | ✅ | `BackupService` 与 `sndb backup` 已落地。 |

## 当前推进顺序

1. M41 P0/P2/P3 作为生产稳定性最高优先级：#368~#380 的性能合同、快速路径、关系输入下推、流式算子、covering/index-only、KV/Table 快照、统计成本模型、EXPLAIN/JOIN 优化、阻塞算子 spill、受控并行和运行时反馈已完成本地实现与自动化门禁；木垒同语料、固定硬件、7 天 mixed workload 和生产 gate 仍为 `NOT_RUN`。下一步按既定顺序推进 #381，不得用增加 SQL permit、内存或索引数量代替根因修复。
