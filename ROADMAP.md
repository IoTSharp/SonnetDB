...
| 29 | 多模型统一管理工作台（Multi-Model Management Workbench） | #245 ~ #260 | 🚧（#245~#254 ✅；Web Admin 旗舰优先） |
...
| 33 | 时序聚合执行与下推优化（aggregate execution & pushdown） | #282 ~ #287 | 进行中（#282 跨字段 Geo 正确性止血 ✅；#283 同字段多聚合单次扫描 ✅；#284 残差/Geo 聚合流式化 ✅；#285~#287 执行/下推优化待做） |
...
> **旗舰（要开始做的）**：
> - **Milestone 29 — 多模型统一管理工作台**：把管理工具从三个孤立工程重构为「一张能力矩阵 × 三个交付面（Web Admin 旗舰 / Studio 桌面 / VS Code）」。**Web Admin 旗舰优先**：A 阶段管理契约 + 统一外壳、B 关系工作台、C KV/MQ/向量已落地（#245~#254 ✅），下一步推进全文 playground #255，再进入 D 对象/文档收口与 E 桌面/VS Code。管理界面跨里程碑归口见 M29「管理界面归口」表。
...
>
> **进行中（按带宽穿插）**：
> - **M33 聚合执行与下推优化**：**#282 跨字段 Geo 聚合正确性修复应先行**——`avg(num) WHERE geo_bbox(pos,...)` 当前在快慢两路都静默丢掉 Geo 过滤（只要聚合带跨字段 Geo 就返回错误结果），属正确性缺陷；其余 #283~#287 为执行/下推性能项，按收益/成本比穿插。与 M31 共享快路径门控面，门控收紧需与 M31 #269 协同。
...
| KV / MQ / 向量 / 全文 专用工作台 | Web Admin | M29 #251~#255 | 🚧（#251/#252/#253/#254 ✅） |
...
| #281 | **Studio 与文档收口**：Document 查询构建器、update preview、bulk import/export、索引设计器、change feed viewer 接入 M24/M29 外壳；补 MongoDB-like 使用指南、迁移手册、能力矩阵和示例应用。 | 📋 |
