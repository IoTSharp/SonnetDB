...
| 20 | 多模型 Parity | ❌ | 最近记录窗口 4/7（低于 95%）；启动修复已本地通过，连续七次远程窗口待重跑。 |
...
| 43 | 十四套能力与生态发布总收口 | 🚧 | #382~#384 已完成；#385~#402 的 CDC、流处理、旅程、报告和生态资料仍在队列。 |
...
Parity 场景、适配器、compose 和 verifier 已完成；MinIO pinned tag 已切换到可用镜像并通过本地 PowerShell 7 contract。当前 4/7 窗口仍低于 95%，须重跑完整 light/full，保留容器日志、测试报告、commit SHA 和 raw artifact；失败不能以 `No summary was produced for this run.` 代替证据。
