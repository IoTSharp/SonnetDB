...
| 28 | 可靠性、并发正确性与热路径加固（P0~P5 分阶段） | #189 ~ #244 | 🚧（P0~P3 + P5a ✅；**SonnetMQ 硬化 #230→#234 收官，数据面 🔴 全部关闭（含 MQ3 无界内存/OOM）**；下一步 = P4 索引/向量 或 P5b 全模型帧接入 + MQTT，可按带宽并行；审计 54 项 + P5 新增 MQ/N 专项） |
